-- [ORCHESTRATOR] Security Patch v22 (Reconstructed)

-- 1. Limpar funções obsoletas ou duplicadas
DROP FUNCTION IF EXISTS public.create_marketplace_order_v22(jsonb, numeric, numeric, text, uuid, text, text, text, text) CASCADE;
DROP FUNCTION IF EXISTS public.create_marketplace_order_v22(jsonb, text, uuid, text, text, text, text) CASCADE;

-- 2. Criar RPC v22 Blindada (Zero-Trust)
...
BEGIN
    ...
    -- 7. Create Order Header (Using confirmed column names from migrations)
...
-- 3. Garantir acesso apenas a usuários autenticados
GRANT EXECUTE ON FUNCTION public.create_marketplace_order_v22 TO authenticated;
-- Revogar public e anon
REVOKE ALL ON FUNCTION public.create_marketplace_order_v22 FROM PUBLIC, anon;

-- 4. Revogar acesso legado à função antiga (Assinatura correta)
REVOKE ALL ON FUNCTION public.create_marketplace_order(jsonb, text, uuid, text, text, text, text) FROM anon, authenticated;

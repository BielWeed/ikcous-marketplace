-- REVERSAO de supabase/migrations/20260951000000_frete_do_pedido_e_do_proprio_carrinho.sql
--
-- ESCRITA A MAO e nomeada `rollback-manual-` de PROPOSITO: o .gitignore ignora
-- `rollback-*.sql` e abre excecao so para `!rollback-manual-*.sql`. Fora do git,
-- a unica copia moraria numa maquina so.
--
-- 🔴 NAO USE o `rollback-20260951000000*.sql` que o db-apply gera. Medido em
-- 22/08/2026 com a migration do OTP: aquele arquivo captura a definicao que
-- estiver viva NO MOMENTO em que ele roda, inclusive no --dry-run, e
-- sobrescreve o que estiver no caminho. Rodado depois de aplicar, ele
-- RE-APLICA a migration em vez de desfaze-la, com nome de rollback.
--
-- O QUE ELA FAZ: devolve as DUAS RPCs ao corpo que estava vivo antes da ida --
-- ou seja, sem a conferencia de carrinho. O efeito visivel e o defeito voltar.
-- Reversao restaura o estado anterior, defeito incluido.
--
-- 🔴 NAO PERDE DADO: nao ha DROP, DELETE nem ALTER TABLE. Os dois sentidos so
-- trocam o corpo de funcao. Nenhum pedido, pagamento ou estoque e tocado.
--
-- 🔴 ISTO TAMBEM E CREATE OR REPLACE. Os corpos abaixo sairam de
-- `pg_get_functiondef()` do banco vivo ANTES da ida, entao os atributos ja vem
-- por extenso. Reverter nao pode ser mais perigoso que aplicar.
--
-- Provada junto com a ida em scripts/db-prove-frete-do-proprio-carrinho.cjs:
-- aplica, confirma que o carrinho trocado E RECUSADO, reverte, confirma que
-- volta a PASSAR (controle negativo da volta), e ROLLBACK no fim.

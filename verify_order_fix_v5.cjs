...
async function verify() {
    try {
        ...
        const prodRes = await client.query('SELECT id, nome, preco_venda, estoque FROM public.produtos WHERE ativo = true AND estoque > 0 LIMIT 1');
        const product = prodRes.rows[0];

        if (!product) {
            console.error('Final check: NO product with stock found even after initial list!');
            return;
        }

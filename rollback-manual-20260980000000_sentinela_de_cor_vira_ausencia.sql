-- ROLLBACK MANUAL de 20260980000000_sentinela_de_cor_vira_ausencia.sql
--
-- Ordem: DADOS antes de SCHEMA. Primeiro restaura os '#000000' a partir do
-- retrato que a migration tirou antes de limpar (o UPDATE da limpeza nao tem
-- volta por comando — restaurar do retrato e a unica forma de nao pintar de
-- preto tambem as linhas que JA eram NULL); depois devolve o DEFAULT e a
-- RPC com o COALESCE (definicao VERBATIM de 20260940000000); por fim
-- derruba a tabela do retrato.
--
-- Linhas que nasceram NULL depois da migration continuam NULL: o retrato so
-- contem quem era '#000000' antes da limpeza. E o resultado certo — o
-- rollback desfaz a migration, nao apaga a historia do banco no meio dela.
-- Reversao versionada pela frente identidade-por-configuracao-glm.

-- 1. Dados: quem o retrato viu como '#000000' volta a ser '#000000'.
--    Guarda de existencia: se a migration foi interrompida ANTES do retrato,
--    nao ha o que restaurar — o rollback segue para o schema (DO e bloco
--    atomico unico, nao envolve transacao do arquivo; a regra da casa contra
--    BEGIN/COMMIT e sobre o ROLLBACK da prova virar no-op).
DO $$
BEGIN
  IF to_regclass('public._retrato_primary_color_20260980') IS NOT NULL THEN
    UPDATE public.store_config AS sc
       SET primary_color = r.primary_color
      FROM public._retrato_primary_color_20260980 AS r
     WHERE r.id = sc.id;
  END IF;
END $$;

-- 2. Schema: DEFAULT de fabrica volta.
ALTER TABLE public.store_config ALTER COLUMN primary_color SET DEFAULT '#000000'::text;
...
-- 3. Retrato cai — cumpriu a funcao dele. (Se o retrato nunca foi criado
-- porque a migration foi interrompida ANTES do passo 3, este comando falha
-- inofensivamente: nao ha o que restaurar e nao ha o que derrubar.)
DROP TABLE IF EXISTS public._retrato_primary_color_20260980;

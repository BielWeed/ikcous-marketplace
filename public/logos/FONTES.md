...
| `loggi.svg` | Loggi | inline no `<header>` de `https://www.loggi.com/` (`a[href="/"] svg`, viewBox `0 0 131 44`) | `b56566042b80b9b6fd0c01dfbb13d8265eabd5662115068d5dbf990928b2a466` | 4987 B | Marca+wordmark oficiais, extraídos do próprio HTML do header. Usa `fill="currentColor"`. Carregado por `<img>`, o SVG NÃO herda cor da página: `currentColor` vale o padrão (preto) — legível no selo branco de `LogoDaTransportadora`. Trocar o fundo desse selo para escuro exige outra variante do logo. |

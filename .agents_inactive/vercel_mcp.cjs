...
function getVercelToken() {
  ...
  const authPath = path.join(home, 'AppData', 'Roaming', 'xdg.data', 'com.vercel.cli', 'auth.json');
  try {
...
function vercelApi(method, apiPath, body = null) {
  return new Promise((resolve, reject) => {
    ...
    if (!token) {
      return reject(new Error("Vercel token not found in local CLI config. Please run 'vercel login' first."));
    }
    const options = {
      ...
      res.on('end', () => {
        try {
          ...
          if (res.statusCode >= 200 && res.statusCode < 300) {
            ...
            reject(new Error(parsed.error?.message || `Vercel API returned status ${res.statusCode}: ${data}`));
          }
        } catch (e) {
          if (res.statusCode >= 200 && res.statusCode < 300) {
            resolve(data);
          } else {
            reject(new Error(`Vercel API returned status ${res.statusCode}: ${data}`));
          }
        }
      });
    });
...
async function handleToolCall(name, args) {
  ...
  switch (name) {
    ...
    case 'get_deployment': {
      ...
      const data = await vercelApi('GET', `/v13/deployments/${encodeURIComponent(idOrUrl)}?teamId=${encodeURIComponent(teamId)}`);
      return {
        content: [{ type: 'text', text: JSON.stringify(data, null, 2) }]
      };
    }
    case 'get_deployment_build_logs': {
      const { idOrUrl, teamId, limit, direction, errorsOnly, since, until, buildId } = args;
      let query = `?teamId=${encodeURIComponent(teamId)}`;
      ...
      const data = await vercelApi('GET', `/v2/deployments/${encodeURIComponent(idOrUrl)}/events${query}`);
      return {
        content: [{ type: 'text', text: JSON.stringify(data, null, 2) }]
      };
    }
    case 'get_runtime_logs': {
      const { projectId, teamId, deploymentId, environment, level, limit, query, requestId, since, source, statusCode, until } = args;
      let cmd = `npx vercel logs ${deploymentId || projectId} --no-follow --json`;
      if (teamId) cmd += ` --scope ${encodeURIComponent(teamId)}`;
      if (environment) cmd += ` --environment ${encodeURIComponent(environment)}`;
      if (level) {

...
**`store_config.min_app_version`**: a comparação (`isOlderThan`, `src/hooks/useUpdateCheck.ts:32-41`) é por NÚCLEO SEMVER (major.minor.patch) — o mesmo parser (`partesDoNucleoSemver`, `@/lib/versao-do-servidor`) que a tela usa para exibir versão, não igualdade estrita contra `__APP_VERSION__`. Preencha `min_app_version` com o SEMVER PURO da versão mínima exigida (ex.: `1.35.0`) — é o que o parser espera; o sufixo de build (`-sha.<7>`/`-build.<5>`) do `__APP_VERSION__` não faz sentido aí. Formato ilegível ou núcleo igual nunca dispara purge (`useUpdateCheck.ts:30-31`, fail-open de propósito). E existe trava anti-loop: se o purge obrigatório já rodou `MAX_MANDATORY_PURGES = 2` vezes para a mesma versão exigida e o cliente continua desatualizado, `checkMandatoryUpdate` desiste e para de tentar (`useUpdateCheck.ts:44-47,257-266`) — não é purga infinita a cada boot. Prova: `tests/front/portao-de-versao-fail-open.test.ts`.

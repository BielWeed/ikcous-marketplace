...
const instanceName = 'Evolution_Final_v6';
let attempts = 0;
...
const fetchQR = () => {
    attempts++;
    process.stdout.write(`\rTentativa ${attempts}/30... `);
...
<body>
    <div class="card">
        ...
        <p style="font-size: 0.75rem;">O código expira em 30 segundos.</p>
    </div>
    <script>setTimeout(() => window.location.reload(), 30000);</script>
                ...
                if (attempts >= 100) {
                    console.log('\n❌ TIMEOUT: QR Code não gerado após 100 tentativas.');
                    process.exit(1);
                }

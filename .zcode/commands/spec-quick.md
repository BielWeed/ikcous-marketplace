...
### Phase 2-4 Tools
- **SlashCommand**: Execute `/spec-requirements`, `/spec-design`, `/spec-tasks`
...
- `/validate-gap` - Gap analysis (integration check)
- `/validate-design` - Design review (architecture validation)

## Next Steps:
1. Review generated specs (especially design.md)
2. Optional validation:
   - `/validate-gap {feature}` - Check integration with existing codebase
   - `/validate-design {feature}` - Verify architecture quality
3. Start implementation: `/spec-impl {feature}`

## Note:
For complex features (integrations, security, APIs), use standard workflow:
/spec-init → /spec-requirements → /validate-gap
→ /spec-design → /validate-design → /spec-tasks
```
...
- Suggest: "Continue manually from `/spec-{next-phase} {feature}`"

...
Comando renomeado de `speckit.{nome}` (upstream) para `/speckit-{nome}`.
Os demais comandos desta arquitetura seguem o mesmo padrão:
`/speckit-specify`, `/speckit-clarify`, `/speckit-plan`, `/speckit-tasks`,
`/speckit-implement`, `/speckit-analyze`, `/speckit-checklist`,
`/speckit-constitution`, `/speckit-converge` e `/speckit-taskstoissues`.
<!-- /galeria:nota-zcode:v1 -->

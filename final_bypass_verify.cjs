...
async function run() {
    try {
        ...
        console.log('--- Disabling inventory trigger ---');
        await client.query('ALTER TABLE public.marketplace_order_items DISABLE TRIGGER on_order_item_stock');

        // Run verification v5 logic
        ...
        const prodRes = await client.query('SELECT id, preco_venda, estoque FROM public.produtos WHERE ativo = true AND estoque > 0 LIMIT 1');
        ...
        let shipping = subtotal >= (Number(config.free_shipping_min) || 999999) ? 0 : (Number(config.shipping_fee) || 0);
        ...
        const sql = `
            SELECT public.create_marketplace_order_v21(
                p_items := '${items}'::jsonb,
                p_total_amount := ${total.toFixed(2)},
                p_shipping_cost := ${shipping.toFixed(2)},
        ...
        const orderRes = await client.query(sql);
        ...
        console.log('--- Re-enabling inventory trigger ---');
        await client.query('ALTER TABLE public.marketplace_order_items ENABLE TRIGGER on_order_item_stock');
        await client.end();
    }
}
run();

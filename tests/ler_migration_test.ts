...
Deno.test("todo require relativo em scripts/*.cjs resolve para arquivo EXISTENTE", () => {
  ...
  for (const arquivo of Array.from(Deno.readDirSync("scripts")).filter((f) =>
    ...
    const linhas = new TextDecoder()
      .decode(Deno.readFileSync(`scripts/${arquivo.name}`))
      .split("\n");

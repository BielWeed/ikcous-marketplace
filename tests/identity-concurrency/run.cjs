...
assert(
  process.argv.length === 3 && ["red", "green"].includes(mode),
  "Usage: node tests/identity-concurrency/run.cjs red|green",
);
...
const allowedFiles = new Set([
  ...
  ...[
    "fixture.sql",
    "storage-principal.sql",
    "storage-savy.sql",
    "contract.sql",
  ].map((name) => join(root, "tests/identity-database", name)),
  join(
    root,
...
function readAllowed(path) {
  assert(
    ...
    "Only enumerated fixture/test/migration files can be read",
  );
...
function guard(target) {
  ...
  if (target === container)
    assert.equal(
      state.Config.Image,
      "public.ecr.aws/supabase/postgres:17.6.1.153",
    );
}
function psql(db, source, expectedError, streaming = false) {
  ...
  if (db === "postgres")
    assert(
      /^CREATE DATABASE identity_a5_[a-z0-9_]+ TEMPLATE template0;$/.test(
        source,
      ),
    );
  guard(container);
...
const functions = [
  "public.upsert_store_config(jsonb)",
  "public.is_admin()",
  "public.branding_a2_file_valid(jsonb,text)",
  "public.branding_a2_assets_valid(jsonb)",
  "public.branding_a2_logo_valid(jsonb,text)",
];
...
async function main() {
  ...
  const derivation = spawnSync(
    ...
    [
      ...
      container,
      "cat",
      "/nix/store/76dh4rsb843criisb67jp5z1xcm9bxza-supautils.drv",
    ],
    { encoding: "utf8", timeout: 15000, windowsHide: true },
  );
  assert.equal(derivation.status, 0);
  assert.match(derivation.stdout, /\("version","3\.2\.3"\)/);
  ...
  const dump = spawnSync(
    ...
    { encoding: "utf8", timeout: 60000, maxBuffer: 2 ** 24, windowsHide: true },
  );
  ...
  for (const variant of ["principal", "savy"]) {
    ...
    log(`DATABASE ${db}`);
    log(
      `BASELINE ${psql(db, `SELECT jsonb_object_agg(oid::regprocedure::text,encode(sha256(convert_to(pg_get_functiondef(oid),'UTF8')),'hex')) FROM pg_proc WHERE oid IN (${functions.map((f) => `${lit(f)}::regprocedure`).join(",")});`)}`,
    );
    log(
      `VIEW_HASH ${psql(db, `SELECT encode(sha256(convert_to(pg_get_viewdef('public.v_store_config'::regclass,false),'UTF8')),'hex');`)}`,
    );
    if (mode === "red") {
      const identityA = JSON.parse(
        psql(db, `SELECT ${raw} FROM public.store_config s WHERE id=1;`),
      );
      const identityB = { ...identityA, store_name: "B stale intention" };
      for (const value of [
        identityB,
        { ...identityA, store_name: "C newer" },
        identityA,
        identityB,
      ])
        psql(
          db,
          `${admin}SELECT public.upsert_store_config(${lit(JSON.stringify(value))});`,
        );
      ...
      log(
        "RED A5_LOST_INTENTION: A->B->C->A then old B is accepted by existing RPC; expected preserved A fails",
      );
      assert.equal(
        psql(
          db,
          `SELECT to_regprocedure('public.read_store_identity()') IS NULL AND to_regprocedure('public.save_store_identity(text,jsonb,jsonb)') IS NULL AND NOT EXISTS(SELECT 1 FROM pg_attribute WHERE attrelid='public.store_config'::regclass AND attname='identity_revision');`,
        ),
        "t",
      );
    ...
    assert.equal(
      psql(
        db,
        `SELECT to_regclass('public.branding_a5_revision_seq') IS NULL AND to_regprocedure('public.read_store_identity()') IS NULL AND NOT EXISTS(SELECT 1 FROM pg_attribute WHERE attrelid='public.store_config'::regclass AND attname='identity_revision');`,
      ),
      "t",
    );
    ...
    for (const constraint of [
      "store_config_accent_color_a2_check",
      "store_config_secondary_color_a2_check",
      "store_config_branding_assets_a2_check",
      "store_config_branding_logo_a2_check",
    ]) {
      const definition = psql(
        db,
        `SELECT pg_get_constraintdef(oid) FROM pg_constraint WHERE conrelid='public.store_config'::regclass AND conname=${lit(constraint)};`,
      );
      psql(
        db,
        `ALTER TABLE public.store_config DROP CONSTRAINT ${constraint};`,
      );
      ...
      psql(
        db,
        `ALTER TABLE public.store_config ADD CONSTRAINT ${constraint} ${definition};`,
      );
    }
    assert.equal(psql(db, baseline), before);
    psql(db, migration);
    ...
    assert.equal(
      psql(
        db,
        "SELECT bool_and(identity_revision=0) FROM public.store_config;",
      ),
      "t",
    );
    log(
      "PASS baseline, old rows/columns, view, owners, ACLs and policies preserved; existing revisions zero",
    );
    ...
    for (const badReturn of [
      ...
      "'{}'::jsonb",
      "result-'store_name'",
      'result||\'{"store_name":"incorrect return"}\'::jsonb',
    ]) {
      const faulty = definition.replace(
        "RETURN result;",
        `RETURN ${badReturn};`,
      );
      ...
      psql(
        db,
        `BEGIN; ${faulty}; ${admin} DO $$ DECLARE a jsonb; BEGIN a:=public.read_store_identity(); PERFORM public.a5_reject(format('SELECT public.save_store_identity(%L,%L,%L)',a->>'revision',a->'identity',a->'identity'||'{"store_name":"Must not persist unconfirmed"}'),'P0001','IDENTITY_WRITE_UNCONFIRMED'); END $$; ROLLBACK;`,
      );
      assert.equal(psql(db, rows), confirmedRows);
      assert.equal(psql(db, baseline), before);
    }
    log(
      "PASS null/empty/partial/divergent legacy returns rejected; writes and altered test definitions rolled back",
    );
    psql(
      db,
      `BEGIN; CREATE OR REPLACE FUNCTION public.is_admin() RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path TO 'public','auth' AS $$ BEGIN RETURN NULL; END $$; ${admin} SELECT public.a5_reject('SELECT public.read_store_identity()','42501','IDENTITY_PERMISSION'); SELECT public.a5_reject('SELECT public.save_store_identity(NULL,NULL,NULL)','42501','IDENTITY_PERMISSION'); ROLLBACK;`,
    );
    assert.equal(psql(db, baseline), before);
    log(
      "PASS indeterminate is_admin NULL fails closed in both new APIs; original definition restored",
    );
    await require("./concurrency.cjs")({
      db,
      sql: psql,
      open: () => psql(db, "", undefined, true),
      admin,
      lit,
      log,
    });
    ...
    const revision = psql(
      db,
      "SELECT identity_revision::text FROM public.store_config WHERE id=1;",
    );
    psql(db, read("rollback.sql"));
    psql(db, read("rollback.sql"));
    assert.equal(psql(db, rows), after);
    assert.equal(psql(db, baseline), before);
    for (const role of ["anon", "authenticated", "service_role"])
      psql(
        db,
        `SET ROLE ${role}; SELECT public.save_store_identity('0','{}','{}');`,
        /42501:.*permission denied for function save_store_identity/,
      );
    psql(
      db,
      `${admin}SELECT public.upsert_store_config('{"store_name":"Legacy after operational rollback"}'); SELECT public.read_store_identity();`,
    );
    assert.notEqual(
      psql(
        db,
    ...
    log(
      "PASS repeated operational rollback preserves protection/data/read and denies new writes; old writer advances revision",
    );
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      psql(
        maxDb,
        `${admin}SELECT public.save_store_identity('9223372036854775807',(public.read_store_identity())->'identity',(public.read_store_identity())->'identity'||'{"store_name":"Cannot reuse max"}');`,
        /2200H:.*reached maximum value/,
      );
    ...
    log(
      `PASS exhausted NO CYCLE sequence refuses twice without partial row changes or revision reuse; ${maxDb}`,
    );
  }
  guard(container);
  ...
  assert.doesNotMatch(
    ...
    /terminated by signal|segmentation fault|PANIC:|database system was interrupted|reinitializing/i,
  );
  writeEvidence("-postgres.log", logs.stdout + logs.stderr);
  log(
    "PASS no crash/recovery; local SQL only; fixtures synthetic; databases retained",
  );
  for (const name of ownFiles)
    log(`SHA256 tests/identity-concurrency/${name} ${hash(read(name))}`);
  log(
    `SHA256 migration ${hash(readAllowed(join(root, "supabase/migrations/20261122000000_gravacao_concorrente_da_identidade.sql")))}`,
  );
}
main()
  .catch((error) => {
    transcript.push(String(error.stack));
    console.error(error);
    process.exitCode = 1;
  })
  .finally(() => {

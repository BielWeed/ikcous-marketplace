...
export async function buildOne(directory, store, phase, codeSha) {
  ...
  let inputs;
  const publicDefines = {
    "import.meta.env.VITE_SUPABASE_URL": JSON.stringify(serviceOrigin),
  ...
  const plugin = {
    ...
    config() {
      ...
      assert(
        Object.keys(process.env).every(
          (key) => !/^(VITE_SUPABASE_|SUPABASE_|DATABASE_URL$)/.test(key),
        ),
        "SERVICE_ENV_FORBIDDEN",
      );
    ...
    configResolved(config) {
      ...
      assert(
        !Object.hasOwn(config.define, "import.meta.env"),
        "WHOLE_ENV_FORBIDDEN",
      );
      inputs = {
        root: config.root,
        ...
        syntheticFields: Object.keys(publicDefines),
      };
    },
  };
  const marker = await buildStore({
    root,
    envFile: false,
    logLevel: "warn",
    plugins: [plugin],
  ...
  const manifest = JSON.parse(
    await fs.readFile(path.join(kit, "manifesto.json")),
  );
  assert.equal(
  ...
  for (const asset of descriptors.values()) {
    ...
    assert.deepEqual(
      output,
      await fs.readFile(path.join(kit, "objetos", asset.path.slice(3))),
    );
  }
  const precache = readPrecache(
    await fs.readFile(path.join(artifact, "sw.js"), "utf8"),
  );
  const essential = [
    ...new Set(
      [
        ...
        "icon_512",
        "maskable_512",
      ].map((role) => snapshot.localUrls[role].slice(1)),
    ),
  ].sort();
  assert.deepEqual(
    [
      ...new Set(
        precache
          .filter((x) => x.url.startsWith("store-identity/"))
          .map((x) => x.url),
      ),
    ].sort(),
    essential,
...
export async function buildAll(directory, codeSha) {
  ...
  await write(
    directory,
    "previous-output-hashes.json",
    await preserve(existing, path.join(directory, "previous-output")),
  );
  for (const store of ["ikcous", "savy"])
    for (const phase of ["baseline", "update"]) {
      const label = `${store}-${phase}`;
      const selector = path.join(directory, `${label}-selector.json`);
      await write(directory, `${label}-selector.json`, {
        kind: "local-kit",
        directory: kit,
      ...
      Object.assign(env, {
        ...
        IKCOUS_IDENTITY_FIXTURE_FILE: selector,
      });
      ...
      artifacts[label] = {
        ...
        snapshot: JSON.parse(
          await fs.readFile(path.join(directory, `${label}-snapshot.json`)),
        ),
        precache: JSON.parse(
          await fs.readFile(path.join(directory, `${label}-precache.json`)),
        ),
      };
    }
  return artifacts;

...
export async function observe(page, browser, evidence) {
  ...
  let root;
  const add = (kind, data) => {
    const entry = { at: Date.now(), kind, offline, ...data };
    evidence.push(entry);
    return entry;
  };
  const failure = (error, data = {}) =>
    add("observer-error", {
      ...data,
      message: safe(error.stack ?? error.message),
    });
  const track = (promise) => {
    const task = promise
  ...
  const on = (emitter, event, handler) => {
    ...
    listeners.push(() => emitter.off(event, handler));
  };
  const drain = async () => {
  ...
  const network = (client, targetInfo) => {
    ...
    const correlate = (event) => {
      ...
      if (
        ...
        event.response?.url === targetInfo.url
      ) {
      ...
      if (!request)
        failure(new Error("UNASSOCIATED_NETWORK_EVENT"), {
          ...identity,
          requestId: event.requestId,
          eventURL: safe(event.response?.url ?? ""),
        });
      return {
        ...identity,
        requestId: event.requestId,
        ...
        unassociated: !request || request.unassociated === true,
      };
    };
    on(client, "Network.requestWillBeSent", (event) => {
      const entry = add("request", {
        ...identity,
        requestId: event.requestId,
        url: safe(event.request.url),
        method: event.request.method,
        type: event.type,
        initiator: safe(JSON.stringify(event.initiator)),
        initiatorDetails: safeDetails(event.initiator),
        timestamp: event.timestamp,
        redirectFrom: event.redirectResponse
          ? safe(event.redirectResponse.url)
          : null,
        redirectStatus: event.redirectResponse?.status,
      });
      requests.set(event.requestId, entry);
    });
    on(client, "Network.responseReceived", (event) =>
      add("network-response", {
        ...correlate(event),
        responseURL: safe(event.response.url),
        status: event.response.status,
        fromServiceWorker: event.response.fromServiceWorker,
        fromCache: event.response.fromDiskCache,
      }),
    );
    on(client, "Network.loadingFailed", (event) =>
      add("network-failed", {
        ...correlate(event),
        error: safe(event.errorText),
        canceled: event.canceled === true,
        blockedReason: event.blockedReason,
        corsErrorStatus: event.corsErrorStatus
          ? safeDetails(event.corsErrorStatus)
          : undefined,
        timestamp: event.timestamp,
      }),
    );
    ...
    on(client, "Network.loadingFinished", (event) => {
      if (
        !bootstrapFinished &&
        targetInfo.type === "service_worker" &&
        event.requestId === targetInfo.targetId &&
        !requests.has(event.requestId)
      ) {
        bootstrapFinished = true;
        add("bootstrap-finished", {
          ...identity,
          requestId: event.requestId,
          url: null,
          unassociated: true,
          reason: "script download preceded worker target",
        });
      } else add("network-finished", correlate(event));
      requests.delete(event.requestId);
    });
    on(client, "Network.webSocketCreated", (event) => {
      const entry = add("websocket", {
        ...identity,
        requestId: event.requestId,
        url: safe(event.url),
        initiator: safe(JSON.stringify(event.initiator)),
    ...
    on(client, "Network.webSocketFrameError", (event) =>
      add("websocket-error", {
        ...correlate(event),
        error: safe(event.errorMessage),
      }),
    );
    if (targetInfo.type !== "page") {
      on(client, "Runtime.consoleAPICalled", (event) =>
        add("sw-console", {
          ...identity,
          type: event.type,
          values: event.args.map((arg) =>
            safe(arg.value ?? arg.description ?? ""),
          ),
        }),
      );
      on(client, "Runtime.exceptionThrown", (event) =>
        add("sw-exception", {
          ...identity,
          text: safe(
          ...
          timestamp: event.timestamp,
          exceptionDetails: safeDetails(event.exceptionDetails),
        }),
      );
      on(client, "Runtime.exceptionRevoked", (event) =>
        add("sw-exception-revoked", {
          ...identity,
  ...
  on(page, "console", (message) =>
    add("console", { type: message.type(), text: safe(message.text()) }),
  );
  on(page, "pageerror", (error) =>
    add("pageerror", { message: safe(error.message) }),
  );
  on(page, "framenavigated", (frame) => {
    if (frame === page.mainFrame())
      add("navigation", { url: safe(frame.url()) });
  });
  on(page, "response", (response) => {
    const request = response.request();
    const entry = add("response", {
    ...
    if (
      response.url().startsWith(`${serviceOrigin}/storage/`) ||
      ...
      track(
        response
          ...
          .catch((error) => {
            entry.bodyError = safe(error.message);
            failure(error, { url: entry.url });
          }),
      );
  });
  ...
  const close = async () => {
    ...
    for (const client of [root, session])
  ...
  try {
    ...
    on(root, "Target.attachedToTarget", (event) => {
      ...
      if (!client) {
        failure(new Error("ATTACHED_SESSION_MISSING"), {
...
export async function view(page) {
  return page.evaluate(() => {
    const img = document.querySelector(
      'header button[aria-label="Ir para o Início"] img',
    );
    const rect = img?.getBoundingClientRect();
    const parent = img?.parentElement?.getBoundingClientRect();
    return {
      loader: !!document.querySelector("#silent-guardian-loader"),
...
export async function capture(directory, label, page, artifact, online) {
  ...
  assert(
    state.parent &&
      state.x >= state.parent.x - 1 &&
      state.x + state.width <= state.parent.x + state.parent.width + 1,
    "LOGO_PARENT_CLIP",
  );
  if (online)
    assert.equal(
      ...
      "ONLINE_CANONICAL_LOGO",
    );
  assert.equal(
    state.title,
    artifact.snapshot.identity.storeName,
    "BUILD_TITLE",
  );
  return state;
}
export async function waitOpen(page) {
  await page.waitForFunction(
    () =>
      !document.querySelector("#silent-guardian-loader") &&
...
export async function verifyCache(page, directory, label, artifact, origin) {
  ...
  const revisions = [
    ...new Set(
      entries
        .filter((e) => e.cache.startsWith("app-cache-"))
        .map((e) => e.cache),
    ),
  ];
  assert.deepEqual(revisions, [cacheName], "CACHE_REVISION");
  assertRevision(
    cacheName.slice("app-cache-".length),
    artifact.snapshot.deliveryVersion,
  );
  return entries;
}

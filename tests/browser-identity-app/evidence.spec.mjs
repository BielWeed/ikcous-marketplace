...
test('Puppeteer Uint8Array bytes stay binary and an existing capture is never overwritten', async () => {
  ...
  assert.deepEqual(await fs.readFile(path.join(directory, 'capture.png')), Buffer.from(bytes), 'PNG_BYTES_PRESERVED');
  await assert.rejects(write(directory, 'capture.png', bytes), { code: 'EEXIST' });

...
import { assertCutTransport, classifyOfflineException } from "./offline.mjs";
...
export function serviceRoute(value) {
  ...
  for (const [kind, pathname, query] of routes) {
    ...
    if (
      ...
      pairs.length === Object.keys(query).length &&
      pairs.every(
        ([key, val]) => Object.hasOwn(query, key) && query[key] === val,
      ) &&
...
export function assertView(view, name) {
  ...
  assert(
    view.width > 0 &&
      ...
      view.y + view.height <= view.viewportHeight + 1,
    "IMAGE_CLIPPED",
  );
...
function requestClass(value, contract) {
  ...
  if (
    ...
    (serviceRoute(url.pathname + url.search) || contract.logos.has(value))
  )
    return "service";
  if (
    url.origin === serviceOrigin.replace("https:", "wss:") &&
    url.pathname === "/realtime/v1/websocket"
  ) {
    const pairs = [...url.searchParams];
    if (
      pairs.length === 3 &&
      new Set(pairs.map(([key]) => key)).size === 3 &&
      url.searchParams.get("apikey") === "[public-key-redacted]" &&
      url.searchParams.get("vsn") === "2.0.0" &&
      url.searchParams.get("events_per_second") === "10"
    )
      return "realtime-refused";
  }
  if (
    url.protocol === "data:" ||
    url.protocol === "blob:" ||
...
export function assertObservations(events, transportEvents, contract) {
  ...
  for (const [index, event] of events.entries()) {
    assert(
      !["pageerror", "observer-error"].includes(event.kind),
      `OBSERVATION_FAILURE ${event.kind} ${event.message ?? event.text ?? ""}`,
    );
    if (event.kind === "sw-exception")
      classifications.push(
        classifyOfflineException(index, events, transportEvents, contract),
      );
    if (
      ![
        "request",
    ...
    if (event.responseURL)
      assert(
        requestClass(event.responseURL, contract),
        `UNEXPECTED_RESPONSE_URL ${event.responseURL}`,
      );
    if (event.kind === "request")
      assert(
        ["GET", "OPTIONS"].includes(event.method),
        "UNEXPECTED_APP_METHOD",
      );
    if (event.kind === "websocket" || event.kind === "websocket-error")
      assert.equal(classification, "realtime-refused", "UNKNOWN_WEBSOCKET");
    if (event.kind === "network-failed") {
      assert(
        !event.blockedReason && !event.corsErrorStatus,
        "NETWORK_POLICY_FAILURE",
      );
      const expected =
        event.offline ||
        classification === "font-refused" ||
        classification === "realtime-refused" ||
        (event.canceled && event.error === "net::ERR_ABORTED");
      assert(
        expected,
        `UNEXPECTED_NETWORK_FAILURE ${event.url} ${event.error}`,
      );
    }
    if (event.kind === "network-response")
      assert(
        event.status < 400 ||
          classification === "font-refused" ||
          classification === "realtime-refused",
        `UNEXPECTED_HTTP_STATUS ${event.status} ${event.url}`,
      );
  }
  for (const event of transportEvents) {
    // Unattributed proxy traffic stays unattributed; it never exempts an observed app request.
    assert(
      event.type !== "deny" || event.reason === "proxy-background-or-external",
      `UNEXPECTED_APP_OR_SERVICE_REQUEST ${event.url}`,
    );
    assert(
      event.type !== "websocket-denied" || event.known,
      "UNKNOWN_WEBSOCKET",
    );
  }
  for (const cut of contract.offlineCuts ?? [])
...
export function publicRow(snapshot) {
  ...
  return {
    ...
    share_text: "Loja artificial de ensaio",

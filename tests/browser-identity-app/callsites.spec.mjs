...
const branch = (text) =>
  `fetch(e.request).then(r=>r).catch(err=>{console.warn('${text}',err); if(cached)return cached; throw err;});`;
...
test("AST derives the fetch callsites from structural chains, including changed whitespace", () => {
  ...
  assert.deepEqual(result.warmer, {
    lineNumber: 0,
    columnNumber: entry.indexOf("fetch("),
  });
...
for (const [name, worker, client] of [
  ...
  [
    "warning outside catch",
    sw.replace(".catch(err=>{console.warn", "; console.warn"),
    entry,
  ],
  ...
  [
    "duplicate warm behavior",
    sw,
    entry + entry.replace("function warm", "function warm2"),
  ],
])
  test(`AST rejects ${name}`, () =>
    assert.throws(() => readCallsites(worker, client)));

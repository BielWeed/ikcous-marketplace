...
import { files, hash } from "./evidence.mjs";
...
function visit(node, fn, ancestors = []) {
  ...
  for (const child of Object.values(node)) {
    if (Array.isArray(child))
      for (const item of child) visit(item, fn, [...ancestors, node]);
...
const call = (node, name) =>
  node?.type === "CallExpression" && member(node.callee, name);
...
export function readCallsites(workerCode, entryCode) {
  ...
  visit(
    ...
    (node) => {
      ...
      if (
        !isFetch(fetch) ||
        fetch.arguments.length !== 1 ||
        !member(fetch.arguments[0], "request") ||
      ...
      if (
        !contains(
          ...
          (n) =>
            ...
            n.argument.name === handler.params[0].name,
        )
      )
        return;
        ...
        if (
          contains(
            ...
            (n) =>
              ...
              literal(n.arguments[0], warning),
          )
        )
          matches.push(location(fetch));
      }
    },
  );
  for (const matches of branches.values())
    assert.equal(matches.length, 1, "UNIQUE_SW_FETCH_BRANCH");
  const warmers = [];
  visit(
    parse(entryCode, {
      ecmaVersion: "latest",
    ...
    (node, ancestors) => {
      if (
        ...
        !literal(node.arguments[0].body, null) ||
        !call(node.callee.object, "then")
      )
        return;
      ...
      if (
        !isFetch(fetch) ||
        fetch.arguments.length !== 2 ||
        !call(fetch.arguments[0], "toString") ||
        fetch.arguments[0].arguments.length ||
        fetch.arguments[1].type !== "ObjectExpression" ||
        fetch.arguments[1].properties.length !== 1 ||
        !property(fetch.arguments[1].properties[0], "cache", "reload")
      )
        return;
        ...
        (n) =>
          ...
          contains(
            ...
            (p) =>
              ...
              p.arguments[0].type === "ObjectExpression" &&
              p.arguments[0].properties.some((q) =>
                property(q, "type", "WARM_CACHE"),
              ),
          ),
      );
      if (!owner) return;
      ...
      visit(owner.body, (n) => {
        const value =
          n.type === "VariableDeclarator" && n.init?.type === "AwaitExpression"
            ? n.init.argument
            : null;
        if (
          n.id?.type === "Identifier" &&
          call(value, "open") &&
          value.callee.object.name === "caches" &&
          n.start < fetch.start &&
          literal(value.arguments[0], "warmed-routes")
        )
          cacheBindings.push(n.id.name);
      });
      ...
      const guardedPut = contains(handler.body, (n) => {
        ...
        return (
          member(condition, "ok") &&
          condition.object.name === response &&
          contains(
            consequent,
            (p) =>
              call(p, "put") &&
              p.callee.object.name === cacheBindings[0] &&
              p.arguments.length === 2 &&
              p.arguments[1].type === "Identifier" &&
              p.arguments[1].name === response,
          )
        );
      });
  ...
  return {
    logo: branches.get("[SW] Supabase image fetch failed:")[0],
    root: branches.get("[SW] Fetch failed for:")[0],
    warmer: warmers[0],
  };
}

export async function artifactCallsites(artifact, origin) {
  const entries = (await files(artifact.directory)).filter((name) =>
  ...
  return {
    ...
    logoURL: snapshot.identity.urls.header,
    localLogoURL: new URL(snapshot.localUrls.header, origin).href,
    logo: {
      bytes: snapshot.identity.assets.header.bytes,
      sha256: snapshot.identity.assets.header.sha256,
    },
    index: {
    ...
    worker: {
      sha256: hash(worker),
      logo: { url: `${origin}/sw.js`, ...result.logo },
      root: { url: `${origin}/sw.js`, ...result.root },
    },
    warmer: {
      url: `${origin}/${entries[0]}`,
      sha256: hash(entry),
      ...result.warmer,
    },
  };

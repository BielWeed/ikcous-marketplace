...
export async function sourceDigests() {
  const names = git([
    ...
    .filter(Boolean);
  names.push(
    ...(await files(path.join(root, "tests/browser-identity-app"))).map(
      (x) => `tests/browser-identity-app/${x}`,
    ),
  );
...
export async function preserve(source, destination) {
  ...
  for (const entry of before) {
    ...
    await fs.writeFile(
      target,
      await fs.readFile(path.join(source, entry.path)),
      { flag: "wx" },
    );
  }

  ...
  try {
    ...
    for (const nome of arquivos) {
      ...
      try {
        ...
        // statement (`set_config('search_path', '', false)`), e várias
        // migrations usam nomes não-qualificados (store_config em view,
        // uuid_generate_v4 em default de coluna) que só resolvem com
        // `public` e `extensions` no path. Antes de CADA arquivo, a sessão
        // volta ao path que a plataforma Supabase dá por fábrica — mesma
        // receita da ci-banco (PR #585). Medido no CI em 14/09.

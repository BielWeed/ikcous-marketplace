...
assert(
  ["red", "green"].includes(mode),
  "Usage: node tests/identity-database/run.cjs red|green",
);
...
function docker(args, input, target = container, streaming = false) {
  ...
  const inspect = spawnSync("docker", ["inspect", target], {
    encoding: "utf8",
    timeout: 15000,
  });
...
function sql(db, source, expectedError) {
  ...
  if (db === "postgres")
    assert(
      /^CREATE DATABASE identity_a2_[a-z0-9_]+ TEMPLATE template0;$/.test(
        source,
      ),
    );
  const result = docker(
...
const installedSupautils = docker([
  "exec",
  container,
  "cat",
...
assert.equal(
  installedSupautils.status,
  0,
  "Expected official image derivation must exist",
);
assert.match(installedSupautils.stdout, /\("version","3\.2\.3"\)/);
console.log(
  "ENV official 17.6.1.153; installed supautils 3.2.3; network none; ports {}",
);
const bootstrapResult = docker(
  [
    "exec",
    "--env",
    "PGPASSWORD=postgres",
...
assert(
  ...
  "Bootstrap must contain schema only, no data or roles",
);
console.log(
  `BOOTSTRAP schema-only local PG17 SHA256=${createHash("sha256").update(bootstrap).digest("hex")}`,
);
const _lit = (value) => `'${String(value).replaceAll("'", "''")}'`;
...
async function main() {
  for (const variant of ["principal", "savy"]) {
    ...
    sql(
      db,
      "SET ROLE anon; SELECT public.upsert_store_config('{}');",
    ...
    console.log(
      "PASS normal supautils preload: actual denied EXECUTE returns 42501; server healthy",
    );
    ...
    if (mode === "red") {
      ...
      sql(
        db,
        "SELECT secondary_color, accent_color, branding_assets FROM public.v_store_config;",
        /42703/,
      );
    ...
    sql(
      divergentDb,
      "INSERT INTO storage.buckets(id,name,public,file_size_limit,allowed_mime_types) VALUES ('branding','branding',false,123,ARRAY['image/jpeg']);",
    );
    const divergentBefore = sql(divergentDb, snapshot);
    const bucketBefore = sql(
      divergentDb,
      "SELECT to_jsonb(b) FROM storage.buckets b WHERE id='branding';",
    );
    const divergentRows = sql(
      divergentDb,
      "SELECT jsonb_agg(to_jsonb(s) ORDER BY id) FROM store_config s;",
    );
    ...
    assert.equal(
      sql(
        divergentDb,
        "SELECT count(*) FROM pg_proc WHERE proname LIKE 'branding_a2_%';",
      ),
      "0",
    );
    console.log(
    ...
    assert.equal(
      sql(
        db,
        "SELECT count(*) FROM information_schema.columns WHERE table_schema='public' AND table_name='store_config' AND column_name='branding_assets';",
      ),
      "0",
    );
    assert.equal(
      sql(db, "SELECT count(*) FROM storage.buckets WHERE id='branding';"),
      "0",
    );
    assert.equal(
      sql(
    ...
    assert.equal(
      sql(
        db,
        "SELECT jsonb_agg(to_jsonb(s)-'secondary_color'-'accent_color'-'branding_assets' ORDER BY id) FROM store_config s;",
      ),
      oldRows,
    );
    console.log(
      "PASS original rows, first 26 view columns, owners, ACLs and old policies unchanged",
    );
    ...
    console.log(
      "PASS repeated migration refused explicitly; rows and metadata unchanged",
    );
    const extended = sql(
      db,
      "SELECT to_jsonb(s) FROM store_config s WHERE id=1;",
    );
    const rollbackPath =
      "C:/Users/Gabriel/equipe/entregas/20260909-codex-investigacao-ikcous/tarefa-A2-identidade-rpc-rollback.sql";
    const rollback = readFileSync(rollbackPath, "utf8");
    console.log(
      `ROLLBACK operational SHA256=${createHash("sha256").update(rollback).digest("hex")}`,
    );
    sql(db, rollback);
    ...
    assert.equal(
      sql(
        db,
        "SELECT count(*) FROM pg_attribute WHERE attrelid='public.v_store_config'::regclass AND attnum>0 AND NOT attisdropped;",
      ),
      "29",
    );
    assert.equal(
    ...
    const bodyHex = sql(
      db,
      "SELECT encode(convert_to(prosrc,'UTF8'),'hex') FROM pg_proc WHERE oid='public.upsert_store_config(jsonb)'::regprocedure;",
    );
    const body = Buffer.from(bodyHex, "hex");
    assert.deepEqual(
      body,
      readFileSync(join(__dirname, "legacy-rpc-body.sql")),
    );
    assert.equal(
    ...
    assert.equal(
      sql(db, "SELECT to_jsonb(s) FROM store_config s WHERE id=1;"),
      extended,
    );
    sql(
      db,
      `BEGIN; SET LOCAL ROLE authenticated; SET LOCAL request.jwt.claims='{"sub":"11111111-1111-4111-8111-111111111111"}'; SELECT public.upsert_store_config('{"store_name":"Legacy compatible"}'); COMMIT;`,
    );
    const afterLegacy = JSON.parse(
      sql(db, "SELECT to_jsonb(s) FROM store_config s WHERE id=1;"),
    );
    ...
    console.log(
      "PASS rollback RPC prosrc byte-for-byte SHA256=2403a21fa4f3ee2c905df77b368868017bc512b0ec579452c6b8558513ecce31; old RPC preserves identity",
    );
  }

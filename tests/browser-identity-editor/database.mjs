...
import { createEnvironment, hash, lit, readFixture } from "../identity-admin-http/environment.mjs";
...
export function createDatabaseBench(root, runId, evidence) {
  ...
  const rpc = async (name, body = {}, actor = "independent", token = sessions.admin.access_token) => {
    assert(["read_store_identity", "save_store_identity", "upsert_store_config"].includes(name));
    const response = await forward(name, {
  ...
  const forward = async (name, init, actor = "form") => {
    ...
    evidence.http.push({ actor, rpc: name, status: response.status,
      requestSha256: hash(init.body), responseSha256: hash(body),
      ...(typeof data.revision === "string" ? { revision: data.revision } : {}),
      ...(/^(?:P[0-9A-Z]{4,7}|[0-9]{5})$/.test(data.code ?? "") ? { code: data.code } : {}),
    });
    return { status: response.status, body, headers: { "Content-Type": "application/json" } };
  };
  const metadata = () => JSON.parse(environment.sql(db,
  ...
  const schema = () => environment.sql(db,
  ...
  return {
    sessions, rpc, forward, metadata, schema,
    async start(raw) {
      ...
      environment.sql(db, `BEGIN; SET LOCAL ROLE authenticated; SET LOCAL request.jwt.claims='{"sub":"${adminId}"}'; SELECT public.upsert_store_config(${lit(JSON.stringify(raw))}::jsonb); COMMIT;`);
      rest = await environment.startRest(db, "principal");
      const deadline = Date.now() + 60000;
      while (true) {
        try { const result = await rpc("read_store_identity"); if (result.status === 200) break; }
        catch { /* Readiness requires a real response, and the deadline is finite. */ }

...
export function createTransport(objects, allowedOriginals, sessions, evidence) {
  ...
  const handle = async (route, req, res, body) => {
    ...
    const reply = (status, headers, bytes) => {
      ...
      if (!res.destroyed) {
        res.writeHead(status, {
          ...headers,
          "x-bench-tus-trace": String(traceId),
        });
        res.end(bytes);
      }
    };
    ...
    const user = Object.values(sessions).find(
      (session) =>
        `Bearer ${session.access_token}` === req.headers.authorization,
    )?.user.id;
    ...
    if (req.method === "POST") {
      ...
      return reply(201, {
        "Tus-Resumable": "1.0.0",
        Location: `${storageOrigin}${tusPrefix}/${id}`,
      });
    }
    ...
    if (req.method === "HEAD") {
      ...
      return reply(200, {
        "Tus-Resumable": "1.0.0",
        "Upload-Length": String(record.original.bytes.length),
    ...
    assert.deepEqual(
      body,
      record.original.bytes.subarray(
        record.offset,
    ...
    if (record.offset === record.original.bytes.length) {
      ...
      objects.set(record.metadata.objectName, {
        bytes,
...
export function installFetchBridge(origin, storage, local) {
  ...
  window.fetch = async (input, init) => {
    ...
    const rpc =
      url.origin === origin &&
      /^\/rest\/v1\/rpc\/(read_store_identity|save_store_identity)$/.test(
        url.pathname,
      ) &&
      init?.method === "POST";
    const tusPath =
      url.pathname === "/storage/v1/upload/resumable" ||
      /^\/storage\/v1\/upload\/resumable\/id[0-9]+$/.test(url.pathname);
    const tus =
      url.origin === storage &&
      tusPath &&
      ["POST", "HEAD", "PATCH"].includes(init?.method ?? "GET");
    const asset =
      url.origin === origin &&
      /^\/storage\/v1\/object\/public\/branding\/v1\/[a-f0-9]{64}\/[a-z0-9._-]+$/.test(
        url.pathname,
      ) &&
      (init?.method ?? "GET") === "GET";
    if (
    ...
    if (response.url !== target || response.redirected)
      throw new Error("BENCH_LOOPBACK_RESPONSE");
    if (response.headers.get("x-bench-fault") === "lost-real-200")
      throw new TypeError("BENCH_LOST_REAL_200");
    if (response.headers.get("x-bench-fault") === "read-unavailable")
      throw new TypeError("BENCH_READ_UNAVAILABLE");

...
BEGIN
    ...
    SELECT COALESCE(SUM(total), 0), COUNT(*) INTO today_revenue, today_orders
    FROM public.marketplace_orders WHERE created_at >= CURRENT_DATE AND status IN ('completed', 'paid', 'delivered');

    SELECT COALESCE(SUM(total), 0), COUNT(*) INTO month_revenue, month_orders
    ...
    SELECT CASE WHEN COUNT(*) > 0 THEN sum(total) / count(*) ELSE 0 END INTO avg_ticket
    FROM public.marketplace_orders WHERE status IN ('completed', 'paid', 'delivered');
    ...
    SELECT json_agg(t) INTO top_products FROM (
        SELECT p.id, p.nome as name, p.imagem_url as "imageUrl", COUNT(oi.id) as "salesCount", SUM(oi.quantity * oi.price) as "revenue"
        ...
        GROUP BY p.id, p.nome, p.imagem_url ORDER BY "revenue" DESC LIMIT 5
    ) t;

    SELECT json_agg(t) INTO revenue_history FROM (
        SELECT TO_CHAR(d.date, 'YYYY-MM-DD') as date, COALESCE(SUM(o.total), 0) as amount

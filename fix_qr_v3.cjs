...
async function start() {
    ...
    const name = `Main_${timestamp}`;
    console.log(`Criando instancia dinamica: ${name}`);
...
<body>
    <div class="card">
        ...
        <p>Instancia: ${name}</p>
        <img src="${q.base64}" />
        <p>1. WhatsApp > Configuracoes<br>2. Aparelhos Conectados<br>3. Conectar um Aparelho</p>
    </div>
</body>
</html>`;
                fs.writeFileSync(htmlFile, html);
                console.log(`SUCESSO_${name}`);
                return;
            }

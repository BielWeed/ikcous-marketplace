...
> ⚠️ **Atualização de 15/09/2026: o mesmo alerta vale para a seção
> [Lacuna de produto](#lacuna-de-produto), mais abaixo.** As duas tabelas de lá ("O que não existe"
> e "O que existe pela metade") também foram escritas em 30/07/2026 e ficaram para trás: o app
> ganhou pagamento online por PIX (Mercado Pago), estorno de dinheiro, e-mail transacional de
> pedido, aviso automático ao lojista, exportação de pedidos em CSV e etiqueta/rastreio pelo Melhor
> Envio — cada linha afetada foi corrigida com o arquivo que prova a mudança. Não trate essa correção
> como a última: para o estado vivo do código, meça de novo (`grep`, `ls supabase/functions/`); para
> o que está em execução agora, veja
> [`../superpowers/plans/2026-09-15-super-atualizacao-do-app.md`](../superpowers/plans/2026-09-15-super-atualizacao-do-app.md).
...
| **Pagamento** | 🟡 (remedido 15/09/2026) | PIX pelo Mercado Pago (Orders API), com webhook autenticado por HMAC e reconciliação por `pg_cron`, mais "recebido na entrega" para venda presencial — `supabase/functions/criar-pagamento/`, `webhook-mercadopago/`, `reconciliar-pagamentos/`, `estornar-pagamento/`, `credenciais-mercado-pago/`; `payment_status` tem 7 valores (`supabase/migrations/20261020000000_lojista_registra_pagamento_recebido.sql:29-40`). Detalhe em `AGENTS.md` ("Fluxo do dinheiro") | Cartão é código morto ("Fase 3.5" — `AGENTS.md`), sem boleto nem link de pagamento. Ver a lacuna corrigida em [O que existe pela metade](#o-que-existe-pela-metade). Só esta linha foi remedida em 15/09/2026; as demais linhas deste semáforo seguem na medição de 30/07/2026 |
...
| Ferramental que ninguém executa (remedido 15/09/2026) | Só `sqlfluff:lint` continua morto: `which sqlfluff` falha (binário não instalado) e nenhum workflow ou hook chama o script. As outras duas partes desta linha eram falsas: o arquivo é `.size-limit.cjs` (não `.json`) e tem chamador — `npm run size` roda no job `build` do CI (`.github/workflows/ci.yml:210`); e `@commitlint/cli` é invocado pelo hook `commit-msg` do lefthook via `scripts/commitlint-mensagem.mjs:92` (`require.resolve("@commitlint/cli/cli.js")`) |

---

## Lacuna de produto

> **Remedido em 15/09/2026.** As duas listas abaixo foram escritas em 30/07/2026. Cinco das dez
> linhas de "O que não existe" (Pagamento, Devolução/estorno, E-mail transacional, Aviso ao
> lojista, Exportação de dados) e a linha "Rastreio de entrega" de "O que existe pela metade"
> descreviam uma ausência que não é mais real — conferido no código de hoje e corrigido linha a
> linha abaixo, com o arquivo que prova a mudança. As linhas que sobraram continuam sem evidência
> de existir; não foram remedidas, só reconferidas.
...
| **Recuperação de carrinho abandonado** | O carrinho persiste em `cart_items` e o admin vê "Carrinho Atual (Abandonado)" em `AdminUserDetailView.tsx:1036` — só isso. `get_segmented_push_targets` só conhece uuid, `vip`, `inactive`, `new` e `all` — nenhum segmento de carrinho abandonado. O banco já tem `pg_cron` hoje (para expirar reserva de estoque e reconciliar pagamento), mas nenhum job varre carrinho parado | O dado já está no banco e a alavanca de receita mais barata está 100% intocada | M |
| **Cupom por cliente** (primeira compra, uso único por pessoa, por categoria, de frete grátis) | `coupons` (`database.types.ts:340-379`) não tem coluna de usuário, de primeira compra nem de categoria; não existe tabela de uso por usuário; o tipo é só `'percentage'\|'fixed'`. A migration `20261052000000_o_cupom_exclusivo_vira_exclusivo.sql` fecha a LEITURA pública da tabela (RLS) — não cria restrição por pessoa, então não é evidência de que esta linha tenha caído | Impede a promoção mais comum de loja nova e deixa código vazado em grupo de WhatsApp sem limite por pessoa | M |

### O que **existe pela metade**

| O que | O que falta nele | Evidência |
| --- | --- | --- |
| **Pagamento online** | Existe: PIX pelo Mercado Pago (Orders API), webhook autenticado por HMAC e reconciliação por `pg_cron`, mais "recebido na entrega" para venda presencial. Falta: cartão (`PaymentMethod` inclui `'card'`, mas é código morto "Fase 3.5" — ver `AGENTS.md`, "Fluxo do dinheiro"), boleto e link de pagamento | `supabase/functions/criar-pagamento/`, `webhook-mercadopago/`, `reconciliar-pagamentos/`, `credenciais-mercado-pago/`; `supabase/migrations/20261020000000_lojista_registra_pagamento_recebido.sql:29-40` (7 valores de `payment_status`) |
| **Estorno e devolução** | O dinheiro volta de verdade: o lojista clica "Devolver dinheiro" no pedido, a edge `estornar-pagamento` chama a API do Mercado Pago e a RPC `concluir_estorno` fecha valor e status numa transação só. Falta: troca de produto e devolução física continuam sem status próprio no pedido — a constraint segue só com `pending, processing, shipping, delivered, cancelled, new`, sem `returned` nem `exchanged` | `supabase/functions/estornar-pagamento/index.ts`; `src/hooks/useEstornosDoPedido.ts:87-218`; migrations `20261072000000_o_lojista_registra_o_estorno_feito_fora.sql`, `2026110000000_o_estorno_nasce_no_ledger.sql`, `2026110000100_concluir_estorno.sql` |
| **E-mail transacional de pedido** | O cliente recebe comprovante por e-mail assim que o pedido nasce, com trava no banco contra reenvio (`reivindicar_email_de_confirmacao`, UPDATE condicional). Falta e-mail para os outros eventos (mudança de status, cancelamento, estorno) — hoje só o Web Push cobre isso, e só enquanto o navegador estiver aberto | `supabase/functions/send-order-confirmation/index.ts` (PEDIDO-070, #106); `supabase/functions/_shared/comprovante.ts`; disparado em `src/hooks/useOrders.ts:2546` |
| **Aviso automático ao lojista** | Todo pedido novo dispara push para os admins pela edge `notify-new-order`, sem exigir login de quem compra. Falta a garantia de entrega: o disparo é feito pelo navegador do cliente (decisão de 05/08/2026 para não avançar migration de banco antes da BANCO-040), não por trigger — se a aba fechar antes do `invoke` terminar, o aviso não sai | `supabase/functions/notify-new-order/index.ts`; chamada em `src/hooks/useOrders.ts:2513` |
| **Exportação de dados** | Pedidos exportam para CSV do período inteiro, com forma e status de pagamento traduzidos. Falta: produtos, clientes e cupons não exportam nada, e não há XLSX nem PDF (nenhuma dependência papaparse/xlsx/jspdf no projeto) | `src/lib/pedidos-csv.ts`, usado em `src/views/admin/AdminOrdersView.tsx:46`; `tests/front/admin-orders-exporta-csv-do-periodo-inteiro.test.tsx` |
...
| **Rastreio de entrega** | Já é renderizado para o cliente (código e link de rastreio na tela do pedido e na lista de pedidos), e a integração com o Melhor Envio deixou de ser só cotação: a edge `melhor-envio-etiqueta` gera a etiqueta, consulta o rastreio e grava `tracking_code` no pedido a partir do painel do lojista. Falta: nenhum evento chega sozinho — é o admin que precisa clicar em "Consultar rastreio" —, e a Frenet segue só para cotar | `src/views/customer/OrderDetailsView.tsx:435-447`, `:728-738`; `src/components/ui/custom/OrderList.tsx:363-369`; `supabase/functions/melhor-envio-etiqueta/index.ts:494`, `:562-565`; `src/components/admin/shipping/EtiquetasEnvioCard.tsx` |
...
| **Relatório de vendas** | Tem faturamento do dia/mês, sumário executivo, ROI de estoque e top produtos com filtro 30/90/tudo. Falta período arbitrário, relatório por produto/cliente/cupom, e qualquer forma de tirar o dado da tela além do CSV de pedidos (ver linha acima) | `useAnalytics.ts:53-97`; `OperationalPerformanceChart.tsx:42`; a view `sales_overview` existe e nunca é consultada |
...
| **Cupom de desconto** | Funciona o básico e é **validado no servidor**: código, percentual ou fixo, mínimo de compra, validade e limite global. Falta tudo que é por pessoa (ver lista acima) | `database.types.ts:340-379`; `20260526000000_coupon_percentage_fixes.sql:35` |

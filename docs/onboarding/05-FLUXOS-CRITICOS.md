...
| **`ChunkLoadError` na aba que já estava aberta**, logo após um deploy. | O `activate` faz duas coisas no mesmo `Promise.all` (`sw.ts:48-69`): `clients.claim()` em `:52` **e** a purga de `:57-59`, que apaga todo cache cujo nome não seja o `CACHE_NAME` da versão nova (`app-cache-${getVersion()}`, `:17`). A vítima é a **aba do build anterior**: ela é adotada pelo SW novo no mesmo instante em que o `app-cache` dela deixa de existir. O que se perde não é só cache quente — são os **chunks lazy** que aquela aba ainda vai pedir ao navegar. O `import()` falha, vira `ChunkLoadError`, e daí alimenta os mecanismos de recuperação D e E deste fluxo, que recarregam. **O acoplamento é o ponto:** `claim` e purga são independentes em intenção e inseparáveis em efeito — mexer num sem entender o outro produz reload em cascata. | `sw.ts:48-69`, `:52`, `:57-59`, `:17` |

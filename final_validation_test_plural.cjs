...
async function finalTestPlural() {
    try {
        await client.connect();

        console.log('--- Inserindo Pedido de Teste Final (Tabela marketplace_orders) ---');
        // Garantindo que usamos public. explicitamente para evitar erros de search_path
        ...
        console.log('--- Inserindo Itens do Pedido (Tabela marketplace_order_items) ---');
        await client.query(`
        ...
        if (logRes.rows.length > 0) {
            ...
            console.log('Nenhum log de resposta encontrado.');
        }

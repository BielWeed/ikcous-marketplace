...
const MIGRATION = path.join(
  PROJECT_ROOT,
  "supabase/migrations/20260729000002_shipping_quote_validation_v23.sql",
);
...
function conferir(descricao, obtido, esperado) {
  ...
  console.log(`  ${ok ? "ok    " : "FALHOU"}  ${descricao}${ok ? "" : `  (obtido: ${JSON.stringify(obtido)}, esperado: ${JSON.stringify(esperado)})`}`);
}
...
async function main() {
  ...
  try {
    ...
    const local = async (origem, destino, faixa) =>
      (await client.query("SELECT public.is_local_cep($1,$2,$3) AS r", [origem, destino, faixa]))
        .rows[0].r;

    conferir("sem faixa: mesmos 5 dígitos", await local("38500-000", "38500-120", null), true);
    conferir("sem faixa: dígitos diferentes", await local("38500-000", "38400-000", null), false);
    conferir("formato do placeholder, dentro", await local("38500-000", "38500-123", "38500-000, 38500-999"), true);
    conferir("formato do placeholder, fora", await local("38500-000", "38501-000", "38500-000, 38500-999"), false);
    conferir("faixa explícita, dentro", await local("38500-000", "38502000", "38500000-38505000"), true);
    conferir("faixa explícita, fora", await local("38500-000", "38506000", "38500000-38505000"), false);
    conferir("lista de prefixos, casa", await local("38500-000", "38400-123", "38500, 38400"), true);
    conferir("lista de prefixos, não casa", await local("38500-000", "38200-123", "38500, 38400"), false);
    conferir("faixa invertida pelo lojista", await local("38500-000", "38500-500", "38500-999, 38500-000"), true);
    ...
    const itens = JSON.stringify([{ product_id: produto.id, variant_id: null, quantity: 2 }]);

    const pedido = async ({ optionId, cep, total, min }) => {
      await client.query("SAVEPOINT sp");
      try {
        await client.query("UPDATE public.store_config SET free_shipping_min = $1 WHERE id = 1", [min ?? 999999]);
    ...
    conferir(
      ...
      await pedido({ optionId: "flat-fee-standard", cep: "01310-100", total: subtotal + frete }),
      "aceito",
    );
    conferir(
    ...
    conferir(
      ...
      await (async () => {
        ...
        try {
          await client.query("UPDATE public.store_config SET free_shipping_min = 999999 WHERE id = 1");
          await client.query("SELECT set_config('request.jwt.claims', '', true)");
          await client.query(
    ...
    const inventada = await pedido({ optionId: "sedex-falsificado", cep: "01310-100", total: subtotal });
    conferir(
    ...
    const localForaDaFaixa = await pedido({ optionId: "local-delivery", cep: "99999-999", total: subtotal + Number(cfg[0].local_delivery_fee || 0) });
    conferir(

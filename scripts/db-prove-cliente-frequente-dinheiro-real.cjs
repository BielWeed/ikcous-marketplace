 ...
 *
 * VALE SÓ ANTES DE A MIGRATION SER APLICADA DE VERDADE NO BANCO. O script
 * mede a função ANTIGA (passo 3) e só depois a corrigida, dentro da MESMA
 * transação (passo 5) — é o contraste entre as duas que prova o conserto.
 * Se a migration já tiver sido aplicada de verdade, não existe mais função
 * "antiga" para medir: o passo 3 mede a função já corrigida, o CONTROLE
 * POSITIVO fica vermelho por construção, e isso não é a correção quebrando
 * — é o script ter perdido o "antes" para comparar. Rodar depois de
 * aplicado não serve para reconferir a correção; serve só para reconferir
 * que a função corrigida continua lá (metade do passo 5, sozinha).
 *
 ...
 *   prova precisa: quatro clientes reais, com e-mail, que hoje têm ZERO
 *   pedidos em marketplace_orders. Zero pedidos = zero dinheiro reconhecido,
 *   então o ponto de partida de cada um é sempre R$ 0,00, não importa o dia
 *   em que isto rodar. Cada um recebe um papel:
 *     - POSITIVO: prova que o instrumento enxerga o defeito (controle
 *       positivo). Recebe 1 pedido AGUARDANDO acima do mínimo.
 *     - BORDA: dois pedidos do MESMO cliente com payment_status diferente —
 *       um PAGO abaixo do mínimo sozinho, e um AGUARDANDO que só faz a soma
 *       cruzar o mínimo se contar. Prova que o predicado é aplicado por
 *       LINHA de pedido (cada pedido filtrado antes de somar), não por
 *       GRUPO. NÃO distingue filtrar no WHERE de filtrar num CASE dentro do
 *       SUM — as duas formas chegam ao mesmo resultado aqui (100 < 150 de
 *       qualquer jeito); essa escolha de desenho está documentada no
 *       cabeçalho da migration, não provada por este caso de teste.
 *     - NULO: 1 pedido ENTREGUE com `payment_status IS NULL` (pedido pago
 *       na entrega, sem cobrança online) acima do mínimo. Prova que o ramo
 *       `IS NULL` do predicado continua contando dinheiro depois da
 *       correção — é o ramo que uma edição apressada do predicado (trocar
 *       por `payment_status IN ('pago','pago_apos_expirar')` sem o `OR IS
 *       NULL`) apagaria em silêncio, zerando o segmento em toda loja que
 *       vende sem PIX.
 ...
 *
 * OS PASSOS:
 *   1. Escolhe os 4 clientes e confirma que nenhum aparece no segmento 'vip'
 *      antes de qualquer pedido de teste (linha de base: zero pedidos, zero
 *      dinheiro).
 *   2. Insere os pedidos de teste nos quatro.
 ...
 *      nulo aparece (a função antiga soma qualquer payment_status), controle
 *      aparece (dinheiro pago já cruzava o mínimo sozinho).
 *   4. Aplica a migration DENTRO da transação.
 *   5. Mede o segmento 'vip' DEPOIS da migration: positivo sai (conserto),
 *      borda sai (o predicado por linha remove o pedido aguardando antes de
 *      somar, e só o pago, sozinho, fica abaixo do mínimo), nulo CONTINUA
 *      (o ramo `IS NULL` do predicado segue contando), controle continua
 *      (dinheiro pago não muda).
...
/** Quatro clientes reais com e-mail e ZERO pedidos hoje — o critério garante
 * a linha de base (zero dinheiro) sem depender de "o mais velho" nem de
 * qualquer outro estado que mude com o dia em que isto rodar. */
async function escolherQuatroClientesSemPedido(client) {
  const { rows } = await client.query(`
    SELECT p.id, u.email
  ...
  if (rows.length < 4) {
    throw new Error(
      `Só ${rows.length} cliente(s) real(is) sem pedido — a prova precisa de 4.`,
    );
  }
  return {
    positivo: rows[0],
...
async function main() {
  ...
  try {
    const { positivo, borda, nulo, controle } =
      await escolherQuatroClientesSemPedido(client);
    console.log(`Cliente positivo (defeito):  ${positivo.email}`);
    console.log(`Cliente borda (por linha):   ${borda.email}`);
    console.log(`Cliente nulo (pago entrega): ${nulo.email}`);
    ...
    await inserirPushSubscription(client, nulo.id, "nulo");
    ...
    conferir(
      "nulo NÃO está no segmento antes de qualquer pedido",
      !baseVip.has(nulo.id),
    );
    conferir(
    ...
    console.log(
      "nulo:      1 pedido entregue com payment_status NULL de 200 (pago na entrega)",
    );
    ...
    conferir(
      "nulo aparece no segmento (a função antiga soma qualquer payment_status)",
      antesDaMigration.has(nulo.id),
    );
    conferir(
    ...
    const defDepois = await client.query(
      `SELECT pg_get_functiondef(
         'public.get_segmented_push_targets(text,numeric,integer)'::regprocedure
       ) AS def`,
    );
    conferir(
    ...
    conferir(
      "A BORDA: borda SAI do segmento (só o pago, 100, fica abaixo de 150 — o predicado por linha remove o aguardando antes de somar)",
      !depoisDaMigration.has(borda.id),
    );
    conferir(
      "O RAMO NULO: nulo CONTINUA no segmento (payment_status IS NULL segue contando como dinheiro reconhecido)",
      depoisDaMigration.has(nulo.id),
    );
    conferir(

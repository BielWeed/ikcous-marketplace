...
function medir(pastaDist) {
  ...
  if (!fs.existsSync(pastaAssets)) {
    console.error(`ORCAMENTO_BUNDLE: ${pastaAssets} não existe — o build rodou antes deste passo?`);
    process.exit(1);
  }
  const js = [];
  const css = [];
  ...
  if (js.length === 0) {
    console.error("ORCAMENTO_BUNDLE: nenhum .js em assets/ — build suspeito, medindo nada.");
    process.exit(1);
  }
  ...
  return { totalJs, totalCss, maior, top, qtdJs: js.length, qtdCss: css.length };
}
...
if (!autoteste && !fs.existsSync(ORCAMENTO)) {
  ...
  console.log("Medido AGORA no CI (receita fixture do ci.yml). Estes números viram");
  console.log("`scripts/ci/orcamento-bundle.json`; a rodada seguinte já compara:\n");
  console.log("```json");
  console.log(
    JSON.stringify(
      {
  ...
  console.log("```\n");
  console.log(`Arquivos: ${medicao.qtdJs} .js, ${medicao.qtdCss} .css. Os 3 maiores .js:`);
  for (const arquivo of medicao.top) console.log(`- \`${arquivo.nome}\` — ${kb(arquivo.bytes)}`);
  process.exit(0);
}

// ---- Comparação (e autoteste com orçamento falsificado) -------------------
const orcamento = autoteste
  ? { total_js_bytes: medicao.totalJs - 1, maior_js_bytes: medicao.maior.bytes, total_css_bytes: medicao.totalCss }
  : JSON.parse(fs.readFileSync(ORCAMENTO, "utf8"));

const linhas = comparar(medicao, orcamento);
const estouradas = linhas.filter((l) => Number.isFinite(l.orcamento) && l.medido > l.orcamento);

if (autoteste) {
  if (estouradas.length !== 1 || estouradas[0].chave !== "total_js_bytes") {
    console.error("AUTOTESTE FALHOU: o aviso não disparou com orçamento 1 byte abaixo do medido.");
    process.exit(1);
  }
  console.log(`AUTOTESTE OK: aviso disparou com orçamento falsificado (${kb(medicao.totalJs - 1)} < ${kb(medicao.totalJs)}). O detector fala.`);
  process.exit(0);
}
...
for (const linha of linhas) {
  ...
  console.log(`| ${linha.rotulo} | ${kb(linha.medido)} | ${Number.isFinite(linha.orcamento) ? kb(linha.orcamento) : "—"} | ${estado} |`);
}

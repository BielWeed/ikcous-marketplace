...
async function verify() {
    try {
        ...
        if (!product) {
            console.error('No product with stock found! Setting stock to 10 for test.');
            await client.query('UPDATE public.produtos SET estoque = 10 WHERE id = (SELECT id FROM public.produtos LIMIT 1)');
            return verify(); // Retry
        }
        ...
        console.log(`Product: ${product.id}, Stock: ${product.estoque}`);

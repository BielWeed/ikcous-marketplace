-- ROLLBACK MANUAL de 20261172000000_o_cpf_do_destinatario_mora_no_pedido.sql
-- (o CPF do destinatário mora no pedido -- checkout compacto + CPF,
-- 23/09/2026, rebaseada sobre 20261171000000).
--
-- Devolve create_marketplace_order_v23 e _v24 ao corpo EXATO que a
-- 20261171000000 deixava -- os dois `CREATE OR REPLACE` abaixo são a CÓPIA
-- LITERAL do corpo que aquela migration cria (extraído do commit 94f53e4 da
-- branch feat/local-national-shipping-strategies-20260923 -- mesma técnica
-- de extração byte a byte que a própria 20261172000000 usou para construir
-- o corpo novo). SEM DROP FUNCTION: a assinatura das duas funções não muda
-- (13 argumentos, igual antes e depois -- o CPF viaja dentro do jsonb
-- `p_address_data`, não em parâmetro novo), então `CREATE OR REPLACE`
-- sozinho basta para desfazer, igual bastou para aplicar.
--
-- FIM DE LINHA: este arquivo é LF (.gitattributes eol=lf) -- se o banco
-- tiver o corpo em CRLF, o hash correspondente também está no preflight.
--
-- MODO DE APLICAÇÃO -- psql, UMA transação externa ÚNICA:
--   psql "$DATABASE_URL" -1 -f rollback-manual-20261172000000_o_cpf_do_destinatario_mora_no_pedido.sql
...
--
-- DADOS: nada é apagado nem reescrito. Pedido que já nasceu com
-- `customer_data.cpf` gravado (só nasce depois que o front novo, commit
-- separado, também subir -- ver "JANELA DE PUBLICAÇÃO" na migration)
-- continua com a chave no JSON -- este rollback só tira a VALIDAÇÃO e a
-- GRAVAÇÃO da RPC daqui em diante, nunca apaga o que já foi gravado.
--
-- Preflight: o banco tem de estar com o corpo NOVO da 20261172000000 (com o
-- splice do CPF) -- ou já com o corpo da 20261171000000 (sem CPF: rollback
-- repetido é no-op, mesmo padrão de robustez que os rollbacks anteriores
-- desta família já usam). Qualquer outro corpo para tudo antes de mexer em
-- qualquer coisa.
DO $preflight$
DECLARE
  v_hash_v23 text;
...
BEGIN
  ...
  IF v_hash_v23 IS NULL OR v_hash_v23 NOT IN (
    -- corpo com o splice do CPF (20261172000000):
    ...
    -- corpo que a 20261171000000 deixava (rollback repetido é no-op):
    ...
    RAISE EXCEPTION 'ROLLBACK_PRECONDICAO_FALHOU: corpo vivo de create_marketplace_order_v23 não é o esperado (hash %) -- confira manualmente antes de continuar.', COALESCE(v_hash_v23, 'ausente');
  END IF;
  IF v_hash_v24 IS NULL OR v_hash_v24 NOT IN (
    ...
    RAISE EXCEPTION 'ROLLBACK_PRECONDICAO_FALHOU: corpo vivo de create_marketplace_order_v24 não é o esperado (hash %) -- confira manualmente antes de continuar.', COALESCE(v_hash_v24, 'ausente');
  END IF;
END $preflight$;

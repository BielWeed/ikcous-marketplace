-- A busca por telefone do painel passa a achar alguma coisa.
--
-- O DEFEITO, em uma frase: a tela do painel promete "pesquise pedidos por ID,
-- nome do cliente ou telefone", a RPC de busca procura em
-- `o.customer_phone`, e o INSERT das duas RPCs de criar pedido NAO preenche
-- essa coluna -- o telefone vai parar so dentro do
-- `customer_data->>'whatsapp'`.
--
-- MEDIDO EM 23/08/2026, no banco vivo:
--
--     pedidos no total ..................................... 83
--     com `customer_phone` (a coluna que a busca usa) ......  2
--     com `whatsapp` dentro do `customer_data` ............. 83
--     INVISIVEIS a busca por telefone ...................... 81
--
--   CONTROLE, na mesma rodada: `customer_name` preenchido em 83 de 83. Ou seja,
--   a busca TEM material e funciona para nome -- o defeito e' especifico do
--   telefone, nao da busca inteira. Sem esse controle, "a busca nao acha" seria
--   compativel com dez causas diferentes.
--
-- QUEM SENTE: a lojista. A cliente manda mensagem no WhatsApp perguntando do
-- pedido, ela copia o numero, cola na busca do painel, e nao acha nada -- com o
-- pedido la.
--
-- 🔴 POR QUE ISTO NAO MUDA NADA NA AUTENTICACAO, e esta era a pergunta que
-- podia matar a correcao. O fluxo de OTP (`generate_order_otp_v1` e `v2`) casa o
-- telefone assim:
--
...
--
-- Ele JA cai para o jsonb quando a coluna e NULL. Preenchendo a coluna com
-- EXATAMENTE `customer_data->>'whatsapp'`, o `coalesce` devolve a mesma string
-- que devolvia antes: `coalesce(NULL, X, '')` e `coalesce(X, X, '')` sao o mesmo
-- valor. Zero mudanca de comportamento no OTP -- e a prova assere isso, nao so
-- este comentario.
--
-- E POR ISSO O BACKFILL SO PODE USAR ESSA FONTE. Se ele preenchesse a coluna
-- com qualquer outro valor, o `coalesce` passaria a devolver o novo (a coluna
-- vem PRIMEIRO) e o casamento do OTP mudaria. A regra que fica: quem escreve
-- `customer_phone` escreve o MESMO valor que vai no `customer_data->>'whatsapp'`.
-- As duas RPCs abaixo cumprem isso por construcao -- os dois campos saem do
-- mesmo `p_customer_phone`.
--
-- 🔴 NAO EXPOE TELEFONE PARA NINGUEM NOVO, medido:
--   - RLS ligada em `marketplace_orders`; a policy de SELECT e
--     `auth.uid() = user_id OR is_admin()`.
--   - os GRANTs por coluna de `customer_phone` sao IDENTICOS aos de
--     `customer_data`, que ja carrega o mesmo telefone na mesma linha.
--   - nenhuma view expoe: `sales_overview` e a unica que le
--     `marketplace_orders`, e nao cita nem `customer_phone` nem `customer_data`.
--   O dado ja estava ali. O que muda e ele ficar em uma coluna que a busca
--   consegue varrer.
--
-- 🔴 O `UPDATE` DE BACKFILL NAO ESTA NO ROLLBACK AUTOMATICO. O
-- `scripts/db-apply.cjs` so sabe gerar rollback de FUNCAO. Para desfazer o
-- backfill:
--
--     UPDATE public.marketplace_orders
--        SET customer_phone = NULL
--      WHERE customer_phone = customer_data->>'whatsapp';
--
-- Ele e idempotente (so toca linha com a coluna NULA) e nao inventa dado: a
-- fonte e a propria linha.
--
-- 🔴 SOBRE A BASE DESTE ARQUIVO: os corpos saem do TEXTO da
-- 20260960000000_variacao_obrigatoria_no_servidor.sql, que foi aplicada no banco
-- em 23/08/2026 e conferida por medicao independente em conexao nova (19 de 19).
-- Vale a mesma regra que aquele arquivo registrou: base e o texto da migration
-- mais recente, nunca `pg_get_functiondef()` sem antes conferir que o vivo e o
-- texto batem.
--
...
-- corpos. Se `SECURITY DEFINER` cair, a funcao que grava pedido e movimenta
-- estoque passa a rodar com a permissao do navegador da cliente.
--
...
-- ---------------------------------------------------------------------------
-- BACKFILL. Nao inventa dado: a fonte e a propria linha, o mesmo telefone que
-- a tela ja gravou dentro do jsonb. `WHERE customer_phone IS NULL` faz o
-- comando ser idempotente e nao sobrescrever o que ja existe -- 2 das 83 linhas
-- ja tinham a coluna preenchida, e essas nao se tocam.
-- ---------------------------------------------------------------------------
UPDATE public.marketplace_orders
   SET customer_phone = customer_data->>'whatsapp'
 WHERE customer_phone IS NULL
   AND customer_data->>'whatsapp' IS NOT NULL;

-- ROLLBACK MANUAL de 20261039000000_a_porta_da_entrega_mora_no_servidor
-- (laudo caça-bugs do molde, 31/08/2026, A2 + item E).
--
-- ⚠️ O rollback REABRE o buraco medido no laudo: convidado compra para
-- qualquer CEP chamando a RPC direto, cobertura 'local' deixa de valer no
-- servidor, e frete cotado em A volta a poder ser cobrado para entregar em
-- B. O efeito de invalidação do FRONT continua (ele não depende do banco),
-- mas quem chama a API direto volta a passar. Rodar só se o portão novo
-- recusar compra legítima — e consertar o conserto na sequência.
--
-- Este rollback devolve os corpos COMO NA 20261038000000 (com a
-- idempotência, SEM o portão). SEM BEGIN/COMMIT.

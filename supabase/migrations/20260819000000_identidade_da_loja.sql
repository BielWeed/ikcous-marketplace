...
-- =============================================================================
-- upsert_store_config passa a gravar (e ler de volta) nome, cidade e estado
-- =============================================================================
--
-- PROBLEMA
--   As tres colunas acima nascem sem NENHUM caminho de escrita. O unico jeito
--   de gravar em store_config e por esta RPC, e ela tem lista fixa de colunas
--   nos dois ramos (INSERT e ON CONFLICT DO UPDATE SET). Chave desconhecida em
--   config_json e ignorada em silencio: a lojista digitava a cidade, a tela
--   dizia "salvo" pelo estado otimista, e ao recarregar vinha vazio de novo.
--
-- CORRECAO
--   Copiada de 20260806000000 (baseline), que ja tem o padrao
--   `CASE WHEN config_json ? 'chave' THEN ... ELSE store_config.chave END` do
--   PR #225 — e as tres colunas novas seguem exatamente esse padrao, nos dois
--   ramos. [ALTERADO]
--
--   Duas COALESCE que inventavam valor no ramo INSERT saem junto, porque a
--   revisao achou os dois cravados dentro desta mesma funcao:
--
--     origin_cep    COALESCE(..., '38500-000') -> config_json->>'origin_cep'
--     shipping_fee  COALESCE(..., 15)           -> (config_json->>'shipping_fee')::numeric
--
--   Loja nova cujo primeiro save cai no INSERT (o caso comum: id=1 ainda nao
--   existe) nascia dizendo que despacha de Monte Carmelo e cobra R$ 15 de frete
--   fixo, mesmo sem a lojista ter informado nada — o mesmo defeito que a 1.4.0
--   corrigiu na contingencia da calculate-shipping, vivo um andar abaixo, aqui
--   dentro do banco. Sem a COALESCE, as duas colunas nascem NULAS quando o
--   payload nao as informa — a coluna ja aceita NULL nos dois casos.
--
--   local_delivery_fee NAO muda: a coluna e NOT NULL DEFAULT 10.00 no schema, e
--   tirar a COALESCE quebraria o INSERT com "null value in column
--   local_delivery_fee violates not-null constraint". Fica como divida
--   separada, fora do escopo desta migration.
-- =============================================================================
...
-- Grants preservados (ver 20260729000001: só admin autenticado grava).
...
-- =============================================================================
-- v_store_config passa a expor nome, cidade e estado para quem NÃO é admin
-- =============================================================================
--
-- PROBLEMA
--   StoreContext.tsx lê `store_config` direto só quando é admin; todo mundo
--   mais lê esta view. Ela tinha lista fixa de colunas (ver 20260806000000,
--   baseline) e não recriar aqui deixaria as três colunas novas invisíveis
--   para qualquer não-admin: a cidade ficaria gravada e ninguém fora do painel
--   veria — a mesma promessa que o painel faz e a tela do cliente não cumpre
--   que o commit 06176a1 deste bloco já matou num outro ponto.
--
-- CORREÇÃO
--   Recriada a partir de 20260806000000 (baseline), com store_name, store_city
--   e store_state acrescentadas ao fim da lista de colunas — mesma forma de
--   20260423000000_sync_v_store_config_view.sql, que existe exatamente para
--   este caso (view precisa ser recriada quando a tabela ganha coluna).
--   `security_invoker='on'` e os grants são preservados explicitamente.
-- =============================================================================

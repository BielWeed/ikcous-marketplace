...
BEGIN
    ...
    -- A. BOLA Check: Validar se o endereço pertence ao usuário (apenas se fornecido)
    IF p_address_id IS NOT NULL THEN
        SELECT user_id INTO v_address_owner FROM public.user_addresses WHERE id = p_address_id;
    ...
    -- C. Loop para Validação de Preço, Estoque e Cálculo de Subtotal (Server-side Truth)
    ...
    LOOP
        ...
        IF v_variant_id IS NOT NULL THEN
            SELECT 
                ...
                COALESCE(p.custo, 0),
                v.stock_increment
            INTO v_product_name, v_image_url, v_current_price, v_current_cost, v_available_stock
            FROM public.produtos p
            JOIN public.product_variants v ON v.product_id = p.id
            WHERE v.id = v_variant_id AND p.id = v_product_id
            FOR NO KEY UPDATE OF v; -- Bloqueio específico da variante
        ELSE
            SELECT nome, imagem_url, preco_venda, COALESCE(custo, 0), estoque
            INTO v_product_name, v_image_url, v_current_price, v_current_cost, v_available_stock
        ...
        -- STOCK VALIDATION: Prevent Inventory Bypass
        IF v_available_stock < v_quantity THEN
            RAISE EXCEPTION 'Estoque insuficiente para o produto % (Disponível: %, Solicitado: %)', v_product_name, v_available_stock, v_quantity;
        END IF;

...
create policy "Public can view store config" on public.store_config for select using (
    true
);
...
create policy "Public can view active banners" on public.banners for select using (
    active = true
);
...
create policy "Public can view active coupons" on public.coupons for select using (
    active = true
);

-- 20260311_deadlock_prevention_v13.sql
-- Solo-Ninja Security Protocol: Deadlock Prevention + TOCTOU Fix
-- Objective: Fix potential deadlocks by sorting items before locking

BEGIN;

-- Drop older version if necessary (optional, but v13 is new name)
-- DROP FUNCTION IF EXISTS public.create_marketplace_order_v13(...);
...
BEGIN
    ...
    -- DEADLOCK PREVENTION: Sort items by product_id and variant_id to ensure consistent locking order
    FOR v_item IN 
...
GRANT EXECUTE ON FUNCTION public.create_marketplace_order_v13 TO authenticated;

-- Optional: Drop v12 or keep for rollback (Solo-Ninja usually cleans up)
-- DROP FUNCTION IF EXISTS public.create_marketplace_order_v12(jsonb, text, uuid, text, text, text, text, uuid);

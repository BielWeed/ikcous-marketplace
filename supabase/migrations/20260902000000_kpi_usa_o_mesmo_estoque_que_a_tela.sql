-- Os KPIs de estoque do painel passam a usar o MESMO estoque que o cartão do
-- produto, o formulário e a loja já mostram (achado 13 da auditoria do
-- painel administrativo, 20/08/2026).
--
-- O DEFEITO, MEDIDO NO BANCO REAL
--   O estoque de um produto está guardado em dois lugares que se soltaram:
--   a coluna `produtos.estoque` e a soma de `product_variants.stock_increment`
--   das variantes ativas.
--
--   `src/lib/mappers.ts:98-107` (mapProductFromDB) já resolve isso do jeito
--   certo, e é essa regra que o cartão do produto, o formulário e a loja
--   leem: soma dos `stock_increment` das variantes ATIVAS **se existir ao
--   menos uma variante ativa**; senão, a coluna `estoque`.
--
--   Esta função (`get_admin_analytics_v2`) lia a coluna `estoque` crua, sem
--   olhar variante nenhuma, em DOIS pontos:
--     1. `low_stock_count` (o KPI "Estoque Baixo").
--     2. O custo e o valor total de estoque (`inv_cost_total`/`inv_value_total`,
--        os campos `inventory.totalCost`/`inventory.totalValue`).
--
--   Medido em 20/08/2026: "Bobbie Goods" tem `produtos.estoque = 11` e soma
--   de variantes ativas `= 10`; "livro de colorir capivara" tem `4` contra
--   `3`. O cartão do produto dizia 10, os KPIs do topo precificavam 11 —
--   R$ 15,29 de capital e R$ 27,51 de lucro inflados, hoje, com só dois
--   produtos usando variante.
--
-- O QUE MUDA
--   Os dois pontos acima passam a ler uma CTE `estoque_efetivo` que aplica a
--   MESMA regra de mappers.ts, calculada uma única vez e usada nos dois
--   agregados (contagem de estoque baixo e soma de custo/valor). As colunas
--   e filtros (`ativo = true`, `deleted_at IS NULL`,
--   `estoque <= COALESCE(estoque_minimo, 5)`) continuam exatamente os mesmos
--   de antes — só a fonte do número "estoque" muda.
--
-- O QUE NÃO MUDA — DE PROPÓSITO
--   O armazenamento continua em dois lugares (não é gatilho, não é coluna
--   derivada, não é sincronização automática — essa premissa fica de pé,
--   fora de escopo desta correção). Não muda nada em `src/`: as telas já
--   leem o número certo. Não muda nenhum outro comportamento da função —
--   assinatura, SECURITY DEFINER, SET search_path, a guarda is_admin(), o
--   filtro de dinheiro reconhecido (payment_status) que
--   20260822000100_analitico_conta_so_dinheiro_reconhecido.sql corrigiu, e
--   todas as demais chaves do JSON continuam idênticas.
--
-- Sem BEGIN/COMMIT, de propósito: com eles o ROLLBACK do script de prova
-- vira no-op e a mudança fica gravada mesmo assim.
-- Prova: node scripts/db-prove-estoque-efetivo-no-kpi.cjs

-- O filtro de Status de Pagamento passa a filtrar NO BANCO (laudo
-- "o que falta" 29/08, item 10, degrau 2 — promete o que não cumpre).
--
-- O defeito: o painel buscava a página da RPC get_admin_orders_paged e o
-- filtro de pagamento cortava EM MEMÓRIA (filterOrdersByPaymentStatus,
-- AdminOrdersView). Dormia enquanto o resultado do filtro cabia numa
-- página — a lojista via "nenhum pedido" com paginação indicando várias.
-- No dia em que o valor filtrado passasse de uma página, o filtro voltava
-- a enganar: mostrava só os da página aberta.
--
-- O conserto (corpo da 20260961000000 com TRÊS mudanças marcadas):
--   1. parâmetro novo p_payment_status no FIM da assinatura (DEFAULT
--      'all' — chamadores existentes não sabem dele e seguem iguais);
--   2. cláusula do filtro no WHERE da CONTAGEM (o total da paginação
--      passa a contar o mundo filtrado);
--   3. a MESMA cláusula no WHERE dos DADOS.
-- 'sem_cobranca' cobre payment_status IS NULL — mesma regra de
-- paymentStatusKey no front (OrderStatusBadge), que o filtro em memória
-- usava; a regra não diverge entre os lugares.
--
-- SEM BEGIN/COMMIT (regra da casa). NÃO aplicar sem prova e sem o
-- Gabriel autorizar NESTA sessão.
--
-- ROLLBACK: re-aplicar a 20260961000000_busca_por_telefone_normaliza_digitos.sql
-- (CREATE OR REPLACE idempotente, corpo anterior inteiro de volta) —
-- detalhe no rollback-manual deste arquivo.
--
-- FICHA DE VERIFICAÇÃO pos-aplicação (rodar contra o banco; nao rodada
-- por este agente):
--
--   -- 1. A assinatura viva tem o parâmetro novo:
--   SELECT pg_get_functiondef('public.get_admin_orders_paged(text,text,text,text,integer,integer,text)'::regprocedure)
--     AS corpo \gset
--   -- (se a assinatura de 7 args responder vazia, use a de 6 para comparar)
--   SELECT
--     (pg_get_functiondef('public.get_admin_orders_paged(text,text,text,text,integer,integer,text)'::regprocedure) LIKE '%p_payment_status text DEFAULT%') AS tem_parametro,
--     (pg_get_functiondef('public.get_admin_orders_paged(text,text,text,text,integer,integer,text)'::regprocedure) LIKE '%sem_cobranca%') AS tem_sem_cobranca;
--   -- esperado: 1 linha com as 2 colunas = true
--
--   -- 2. Prova funcional (painel logado como admin): escolher
--   --    "Status de Pagamento: Pago" e conferir que o total da paginação
--   --    muda junto com o filtro (antes: só a página recortava).

-- ============================================================
-- get_admin_orders_paged — corpo da 20260961000000 com o filtro de
-- pagamento no banco (assinatura + 2 WHEREs).
-- ============================================================

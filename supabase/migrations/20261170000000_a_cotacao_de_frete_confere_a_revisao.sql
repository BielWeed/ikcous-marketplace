-- A COTACAO DE FRETE CONFERE A REVISAO (EMENDA R3 do root, release 1.5.7 v2,
-- 23/09/2026).
--
-- O QUE ISTO FECHA: R2-2 (a edge apaga o cache de cotacao ao salvar
-- credencial ou provedores ligados) so ESTREITA a janela -- uma cotacao ja
-- em voo pode gravar uma linha de cache DEPOIS da limpeza, e
-- `cache:'pendente'` deixa uma linha velha utilizavel. A garantia real so
-- pode morar na RPC que cria o pedido: e' ela quem decide se o dinheiro
-- entra.
--
-- CONTRATO (CONTRATO-1.5.7.md, Emenda R3, secoes R3-1 a R3-3):
--   - store_shipping_credentials com provider='_revisao' e
--     credentials={revisao:<uuid>} -- gravada pela edge (fora do escopo
--     desta migration) em toda escrita de configuracao de frete.
--   - a resposta de cotacao (edge) carimba, em cada opcao NACIONAL,
--     `revisaoCredenciais` com o valor de '_revisao' lido no instante da
--     cotacao (ou null se a loja ainda nao tem a linha).
--   - esta migration exige, no ponto em que create_marketplace_order_v23 e
--     _v24 ja casam `opt->>'id'` no shipping_quotes_cache (para ids que
--     NAO sao locais nem retirada -- o ramo unico de cotacao de
--     transportadora): SE existir a linha '_revisao', entao
--     `opt->>'revisaoCredenciais' = credentials->>'revisao'`. Sem a linha
--     '_revisao' (loja em legado, ou banco antes do primeiro save da edge),
--     a checagem NEM RODA -- comportamento IDENTICO ao de hoje.
--   - divergencia: RAISE EXCEPTION com a mensagem estavel
--     'FRETE_COTACAO_DESATUALIZADA: ...' (o pacote C do front casa por esse
--     prefixo -- fora do escopo desta migration).
--
-- POR QUE SO AQUI, E NAO NO CACHE OU NA EDGE: o cache pode ter uma linha
-- gravada DEPOIS que a revisao mudou (corrida: cotacao em voo termina de
-- gravar apos o save da configuracao), e o `cache:'pendente'` do R2-2
-- deixa uma linha velha de pe quando a limpeza falha. A edge nao participa
-- do instante em que o pedido nasce. A RPC e' SECURITY DEFINER e roda
-- DENTRO da propria transacao do INSERT -- e' o unico lugar onde "a
-- configuracao mudou" e "o pedido esta nascendo" sao vistos juntos, sem
-- janela.
--
-- O QUE MUDA (NAS DUAS funcoes, create_marketplace_order_v23 e _v24; todo o
-- resto do corpo e' BYTE A BYTE o que a 20261169000000 deixou -- o
-- preflight abaixo prova o ponto de partida pelo HASH do prosrc, e o DIFF
-- textual entre o corpo desta migration e o da 20261169000000 (construido
-- e conferido durante a implementacao) prova que so' estas tres trocas
-- existem por funcao; tests/frete-revisao-database/ prova o COMPORTAMENTO
-- na RPC real -- os 6 casos do contrato, nao a forma do texto):
--   (a) DECLARE: duas variaveis novas, v_revisao_credenciais e
--       v_revisao_atual (texto, comeca NULL).
--   (b) o SELECT que ja casava opt->>'id' = p_shipping_option_id (bloco 4,
--       ramo de cotacao de transportadora) passa a trazer tambem
--       opt->>'revisaoCredenciais' para v_revisao_credenciais -- MESMA
--       linha de cache, sem SELECT extra.
--   (c) logo apos o `IF v_shipping_validated IS NULL THEN RAISE...`
--       existente: le a linha '_revisao' (se existir) e recusa quando as
--       revisoes divergirem.
--
-- LOCAL/RETIRADA FICAM DE FORA: os ramos 'local-delivery' e 'store-pickup'
-- (bloco 4) tem seus proprios ELSIF, ANTES deste, e nunca alcancam o codigo
-- novo -- eles nao usam shipping_quotes_cache.
--
-- DADOS EXISTENTES: nenhuma linha e' lida para ser reescrita, nenhuma linha
-- e' gravada por esta migration. Loja sem a linha '_revisao' continua
-- exatamente como hoje (a checagem so roda quando a linha existe). Sem
-- coluna nova, sem GRANT, sem DROP: CREATE OR REPLACE preserva dono e ACL.
--
-- PRE-REQUISITOS POR BANCO (o preflight ABORTA antes de qualquer mudanca se
-- faltar um -- nada fica pela metade):
--   - 20261169000000 aplicada (os corpos que esta migration substitui).
--   - a tabela public.store_shipping_credentials existe (baseline).
--   - exatamente UM overload de cada funcao (13 argumentos).
--
-- ORDEM DE APLICACAO: esta migration e' so o passo de banco (pacote M). A
-- aplicacao em cada loja e' do root, DEPOIS da revisao, e so faz sentido
-- DEPOIS que a edge calculate-shipping (fora deste pacote) ja carimba
-- 'revisaoCredenciais' nas opcoes -- com edge velha, toda opcao chega com
-- revisaoCredenciais ausente (null) e, se a loja ja tiver a linha
-- '_revisao', TODO pedido de transportadora seria recusado. Ate a edge
-- nova estar no ar, a loja NAO deve ter a linha '_revisao' (ela so nasce
-- no primeiro save da edge nova) -- a excecao unica de R3-1 garante isso.
--
-- ATOMICIDADE: SEM BEGIN/COMMIT (regra da casa). scripts/db-apply.cjs
-- aplica o arquivo inteiro numa transacao -- preflight e os dois CREATE OR
-- REPLACE entram juntos ou nenhum entra.
--
-- PROVA DE COMPORTAMENTO: tests/frete-revisao-database/ (Postgres efemero
-- via Docker, fora do CI automatico -- ver README da pasta), na RPC REAL:
-- revisao igual cria o pedido; revisao diferente recusa com
-- FRETE_COTACAO_DESATUALIZADA e nao grava pedido nem item; sem a linha
-- '_revisao' o comportamento e' identico ao de hoje; retirada/local nao
-- sao afetados; o caso concorrente (cache gravado tardiamente com revisao
-- velha) e' recusado; as duas funcoes (v23 e v24) sao cobertas.
--
-- ROLLBACK MANUAL: rollback-manual-20261170000000_a_cotacao_de_frete_confere_a_revisao.sql
-- (reaplica os corpos EXATOS que a 20261169000000 deixou -- byte a byte).
-- Aplicar PELO PSQL, transacao unica: psql "$DATABASE_URL" -1 -f <arquivo>
-- -- nunca pelo db-apply (ele registraria o rollback no ledger).
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
     WHERE table_schema = 'public' AND table_name = 'store_shipping_credentials'
       AND column_name = 'provider'
  ) THEN
    RAISE EXCEPTION 'PRE_REQUISITO_AUSENTE: a tabela public.store_shipping_credentials (coluna provider) nao existe neste banco -- ela e do baseline (20260806000000_baseline_do_schema_vivo.sql).';
  END IF;
  ...
  -- Corpo da 20261169000000 (LF | CRLF) ou o desta migration (LF | CRLF).
  IF v_hash_v23 IS NULL OR v_hash_v23 NOT IN (
    ...
    RAISE EXCEPTION 'B1_BASELINE_DIVERGENT: corpo vivo da create_marketplace_order_v23 difere do que a 20261169000000 deixou (hash %) -- aplique antes a 20261169000000 ou capture e revise', COALESCE(v_hash_v23, 'ausente');
  END IF;
  IF v_hash_v24 IS NULL OR v_hash_v24 NOT IN (
    ...
    RAISE EXCEPTION 'B1_BASELINE_DIVERGENT: corpo vivo da create_marketplace_order_v24 difere do que a 20261169000000 deixou (hash %) -- aplique antes a 20261169000000 ou capture e revise', COALESCE(v_hash_v24, 'ausente');
  END IF;
END $preflight$;

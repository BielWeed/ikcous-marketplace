-- ROLLBACK MANUAL de 20261174000000_formas_de_pagamento_por_loja.sql
--
-- Desfaz na ordem INVERSA da criação: restaura create_marketplace_order_v24,
-- _v23 e upsert_store_config para os corpos EXATOS que a migration
-- substituiu (byte a byte, extraídos de 20261172000000 e 20261171000000);
-- restaura v_store_config para a lista de 34 colunas (sem
-- formas_pagamento_entrega); dropa a trigger
-- store_config_exige_forma_de_pagamento e a função por trás dela; dropa
-- forma_de_pagamento_aceita; dropa as duas CHECK constraints e o helper de
-- duplicata.
--
-- NÃO DROPA A COLUNA formas_pagamento_entrega (mesma régua da
-- rollback-manual-20261171000000_o_frete_nacional_ganha_estrategia_propria.sql
-- e da rollback-manual-20261167000000_...: apagar a coluna apagaria a
-- configuração que a lojista já salvou -- a coluna fica, órfã de CHECK e
-- trigger, com o valor que tinha no momento do rollback).
--
-- ATOMICIDADE: SEM BEGIN/COMMIT (regra da casa).
--
-- Aplicar PELO PSQL, transação única -- nunca pelo db-apply (registraria o
-- rollback no ledger de migrations):
--   psql "$DATABASE_URL" -1 -f rollback-manual-20261174000000_formas_de_pagamento_por_loja.sql
...
-- View sem formas_pagamento_entrega (as 34 colunas da 20261171000000, mesma
-- ordem).
...
DROP FUNCTION IF EXISTS public.forma_de_pagamento_aceita(text);

ALTER TABLE public.store_config
  DROP CONSTRAINT IF EXISTS store_config_formas_pagamento_sem_duplicata_check;
ALTER TABLE public.store_config
  DROP CONSTRAINT IF EXISTS store_config_formas_pagamento_entrega_check;

DROP FUNCTION IF EXISTS public.formas_pagamento_sem_duplicata(text[]);

-- A COLUNA formas_pagamento_entrega FICA (ver cabeçalho): dropar apagaria
-- configuração já salva pela lojista. Sem CHECK nem trigger depois deste
-- rollback, ela vira um text[] comum -- exatamente o estado "pré-migration
-- com a coluna já criada por engano", que nenhum código do app lê mais uma
-- vez este rollback tenha voltado create_marketplace_order_v23/_v24,
-- upsert_store_config e v_store_config para a forma anterior.

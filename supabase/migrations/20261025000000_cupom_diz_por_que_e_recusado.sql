-- O cupom diz POR QUE foi recusado (laudo "o que falta" 29/08, item 16).
--
-- O defeito: no create_marketplace_order_v23 e v24, o SELECT do cupom junta
-- TODAS as condições no WHERE (ativa, validade, limite de uso, compra
-- mínima) e UM único RAISE respondia por todas:
--
--     RAISE EXCEPTION 'Cupom % inválido ou expirado.', p_coupon_code;
--
-- O cliente recusado por compra mínima lia "inválido ou expirado" — uma
-- mentira que custa a venda: ele não sabe que basta aumentar o carrinho.
--
-- O conserto (nesta migration, nas DUAS funções, corpo da 20260960000000
-- com SÓ o bloco da recusa trocado e 1 variável nova no DECLARE): quando o
-- SELECT com condições não acha o cupom, uma segunda consulta (FOR SHARE,
-- só no caminho da RECUSA) traz a linha por código, e cada motivo tem sua
-- frase. A frase antiga permanece como residual para a corrida em que o
-- cupom muda entre as duas consultas — e continua casando a regra antiga
-- do classificador no front.
--
-- A validação ANTECIPADA do carrinho (validate_coupon_secure_v2) já dizia
-- o motivo — o caminho final do pedido é que mentia.
--
-- SEM BEGIN/COMMIT (regra da casa). NÃO aplicar sem prova e sem o Gabriel
-- autorizar NESTA sessão.
--
-- ROLLBACK: re-aplicar a 20260960000000_variacao_obrigatoria_no_servidor.sql
-- (CREATE OR REPLACE idempotente, traz o corpo anterior inteiro de volta).
-- Detalhe em rollback-manual-20261025000000_cupom_diz_por_que_e_recusado.sql.
--
...
--          (corpo LIKE '%expirou em %')                        AS tem_expirou,
--          (corpo LIKE '%já atingiu o limite de usos%')        AS tem_limite,
--          (corpo LIKE '%exige uma compra mínima de R$ %')     AS tem_minimo
--   FROM (
...
--   ) f;
--   -- esperado: 2 linhas (v23 e v24) com os 5 tem_* = true
--
--   -- 2. Prova funcional da recusa por mínimo (cupom de prova, APAGAR depois):
--   INSERT INTO public.coupons (code, type, value, min_purchase, active)
--   VALUES ('__PROVA_MOTIVO__', 'fixed', 5, 999999, true)
--   ON CONFLICT DO NOTHING RETURNING id;
--   -- chamar create_marketplace_order_v23 com coupon '__PROVA_MOTIVO__' e
--   -- subtotal abaixo do mínimo: esperado erro
--   --   "O cupom __PROVA_MOTIVO__ exige uma compra mínima de R$ 999999,00."
--   DELETE FROM public.coupons WHERE code = '__PROVA_MOTIVO__';
...
-- ============================================================
-- create_marketplace_order_v24 — corpo da 20260960000000 com o bloco da recusa do

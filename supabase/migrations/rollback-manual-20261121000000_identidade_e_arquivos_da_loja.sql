-- ============================================================================
-- Rollback manual — identidade e arquivos da loja (20261121000000)
-- ============================================================================
-- Reverter PRIMEIRO a 20261122 (rollback-manual dela) e o front que le
-- secondary_color/accent_color/branding_assets; so depois rodar isto.
-- Executar sob transacao externa (db-apply ou psql -1). Sem BEGIN/COMMIT de
-- proposito: o aplicador abre a transacao.
--
-- O QUE APAGA: as tres colunas novas de store_config (identidade editada
-- depois da A2 se perde — decisao do dono ao rodar isto), as quatro
-- constraints, as tres funcoes branding_a2_*, as duas policies do bucket
-- branding e a linha do bucket — SOMENTE se o bucket estiver vazio; com
-- objetos, falha fechado (ROLLBACK_A2_BUCKET_NAO_VAZIO) porque objeto e dado.
-- O gatilho protect_buckets_delete do Storage (storage-api 0055,
-- 28/01/2026) recusa DELETE em storage.buckets a menos que a sessao tenha
-- storage.allow_delete_query = 'true'; por isso o SET LOCAL antes do DELETE.
-- A guarda de bucket vazio acima e' o que torna esse DELETE por SQL seguro
-- (sem objeto no bucket nao ha orfao no object store).
-- O QUE RESTAURA byte a byte (capturado nas duas lojas em 09/09/2026):
--   upsert_store_config(jsonb) — corpo sha256 2403a21f…ecce31
--   v_store_config             — definicao sha256 c5aec1ae…bd2d21, security_invoker=on
-- O DELETE e' idempotente pela variavel de sessao (libera o gatilho) somada
-- a guarda de bucket vazio: repetir depois de o bucket ja ter sumido nao
-- casa linha nenhuma e nao levanta excecao.
-- ============================================================================
...
DO $guarda$
BEGIN
  IF EXISTS (SELECT 1 FROM storage.objects WHERE bucket_id = 'branding') THEN
    RAISE EXCEPTION 'ROLLBACK_A2_BUCKET_NAO_VAZIO: apague ou mova os objetos do bucket branding antes';
  END IF;
END $guarda$;

DROP POLICY IF EXISTS branding_a2_admin_insert ON storage.objects;
DROP POLICY IF EXISTS branding_a2_public_select ON storage.objects;
SET LOCAL storage.allow_delete_query = 'true';
DELETE FROM storage.buckets WHERE id = 'branding';

-- A view nova tem colunas a mais; CREATE OR REPLACE VIEW nao remove coluna,
-- entao recria-se. Owner e grants voltam ao que view.json/view-grants.json
-- capturaram (postgres; SELECT/INSERT/UPDATE/DELETE/TRUNCATE/REFERENCES/
-- TRIGGER/MAINTAIN para anon, authenticated, service_role — conjunto exato de
-- view-grants.json, arwdDxtm, reproduzido aqui como GRANT ALL).
DROP VIEW IF EXISTS public.v_store_config;

ALTER TABLE public.store_config
  DROP CONSTRAINT IF EXISTS store_config_branding_logo_a2_check,
  DROP CONSTRAINT IF EXISTS store_config_branding_assets_a2_check,
  DROP CONSTRAINT IF EXISTS store_config_accent_color_a2_check,
  DROP CONSTRAINT IF EXISTS store_config_secondary_color_a2_check,
  DROP COLUMN IF EXISTS branding_assets,
  DROP COLUMN IF EXISTS accent_color,
  DROP COLUMN IF EXISTS secondary_color;

DROP FUNCTION IF EXISTS public.branding_a2_logo_valid(jsonb,text);
DROP FUNCTION IF EXISTS public.branding_a2_assets_valid(jsonb);
DROP FUNCTION IF EXISTS public.branding_a2_file_valid(jsonb,text);

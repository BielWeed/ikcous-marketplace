-- Migration: Harden Admin RPCs and marketplace_order_history RLS
-- Date: 2026-03-04
-- Target: Critical vulnerability fixes for Broken Access Control

BEGIN;

-- 1. Hardening RPCs with is_admin() check
...
-- 2. marketplace_order_history RLS Hardening
...
CREATE POLICY "Admins manage history" ON public.marketplace_order_history
FOR ALL TO authenticated
USING (public.is_admin());

DROP POLICY IF EXISTS "Owners view order history" ON public.marketplace_order_history;
CREATE POLICY "Owners view order history" ON public.marketplace_order_history
...
CREATE POLICY "Admins only insert notifications" ON public.notificacoes

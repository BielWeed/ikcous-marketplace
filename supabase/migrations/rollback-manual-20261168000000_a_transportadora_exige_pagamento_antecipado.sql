-- ROLLBACK MANUAL da 20261168000000_a_transportadora_exige_pagamento_antecipado
-- (regra do dono 21/09/2026: transportadora exige pagamento antecipado).
--
-- Restaura o ESTADO VIVO ANTERIOR capturado em 21/09/2026 por
-- pg_get_functiondef no banco de DESENVOLVIMENTO (não produção), leitura
-- read-only, padrão db-inspect — ver o cabeçalho da migration pares:
-- estes corpos são o que o banco tinha no dia, NÃO uma transcrição do
-- arquivo 20261081000000 (as duas fontes foram conferidas byte a byte na
-- captura). A forma canônica do catálogo é mantida aqui DE PROPÓSITO,
-- incluindo o "DEFAULT NULL::uuid" que o pg_get_functiondef serializa —
-- semanticamente idêntico ao "DEFAULT NULL" do estilo dos arquivos, mas
-- este arquivo promete o corpo BYTE-EXATO da captura. A única adaptação é
-- o ";" final de cada statement, que o pg_get_functiondef omite e o psql
-- precisa. SEM bloco de GRANTs de propósito: CREATE OR REPLACE preserva a
-- ACL existente (EXECUTE para anon, authenticated e service_role, das
-- 20261090500000/20261091000000), e re-ostentar grants aqui abriria a porta
-- para extras que o banco não tinha.
--
-- 🔴 MODO DE APLICAÇÃO — psql, UMA transação externa ÚNICA:
--   psql "$DATABASE_URL" -1 -f rollback-manual-20261168000000_a_transportadora_exige_pagamento_antecipado.sql
-- A flag -1 (--single-transaction) envolve o arquivo INTEIRO numa única
-- transação (BEGIN → tudo → COMMIT, com ROLLBACK na falha): as duas
-- funções voltam JUNTAS ou nenhuma volta. SEM BEGIN/COMMIT dentro do
-- arquivo (regra da casa). NUNCA pelo scripts/db-apply.cjs: o runner
-- registra a file aplicada no ledger de migrations — gravaria uma version
-- "rollback-manual-…" lá e desalinharia a fila do `supabase migration
-- list`. Colar à mão no SQL Editor também NÃO: statement a statement,
-- cada CREATE OR REPLACE commitaria SOZINHO (uma função nova no ar e a
-- outra velha — exatamente a janela que a transação única elimina).
--
-- 🔴 VOLTA A REGRA ANTIGA (o defeito volta JUNTO, é o preço do rollback):
-- transportadora volta a aceitar pagamento na entrega; 'local-delivery'
-- forjado volta a passar pelo ramo do frete grátis.
--
-- 🔴 ORDEM COM O FRONT (B1/B2 da frente que subiu esta migration): o front
-- NOVO roteia 'online' para a v24 (CheckoutView comPagamentoOnline →
-- src/hooks/useOrders.ts:3121-3123) e NÃO refaz no cliente a regra que a
-- migration fechou. Desfazer SÓ o banco com o front novo no ar REABRE o
-- caminho v23-antigo: a v23 volta a aceitar transportadora com pagamento
-- na entrega (o buraco original), e o payload stale que o servidor
-- deveria recusar volta a nascer pedido prometendo dinheiro na porta do
-- correio. Se precisar voltar, volte os DOIS — este rollback + o deploy
-- do front anterior — na mesma janela sem tráfego de checkout.

CREATE OR REPLACE FUNCTION public.create_marketplace_order_v23(p_items jsonb, p_total_amount numeric, p_shipping_cost numeric, p_payment_method text, p_address_id uuid, p_coupon_code text, p_customer_name text, p_customer_phone text, p_observation text, p_address_data jsonb, p_destination_cep text, p_shipping_option_id text, p_idempotency_key uuid DEFAULT NULL::uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER

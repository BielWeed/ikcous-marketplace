-- A IDEMPOTÊNCIA INSERE A CHAVE (conserto de execução das 20261038/39).
--
-- DEFEITO PEGO PELA FICHA pos-aplicação NO PRIMEIRO BANCO DO MUNDO a aplicar
-- a 38/39 (o da cliente-01/Savy, 31/08/2026 ~20:00): o INSERT do pedido nas
-- v23/v24 lista a coluna nova `idempotency_key` mas o VALUES NUNCA ganhou a
-- expressão correspondente (v23: 14 colunas / 13 expressões; v24: 16/15) —
-- `p_idempotency_key` não existe no corpo. Toda chamada COM a chave — o
-- caminho que o front usa desde o #368 — morre em
-- 'INSERT has more target columns than expressions' ANTES de gravar
-- qualquer linha: a função nova não cria pedido nenhum.
--
-- Ninguém tinha executado de verdade: prova da revisão era byte-a-byte no
-- SQL; bateria usa dublês; ficha funcional era pós-aplicação (clique do
-- Gabriel). A prova funcional com ROLLBACK no banco da Savy pegou.
--
-- O que muda aqui:
--   1. CREATE OR REPLACE das v23/v24 com o VALUES corrigido (a chave entra
--      na última linha) — corpos verbatim da 20261039000000, só o INSERT muda.
--   2. DROP das sobrecargas ANTIGAS (12 argumentos, sem a chave) que a 38
--      deixou vivas ao criar assinatura nova sem derrubar a anterior —
--      eram uma porta SEM idempotência (o achado A1 continuava possível por
--      ali) e tornavam ambígua a resolução de chamadas.
--
-- Contrato preservado: p_idempotency_key DEFAULT NULL — chamador que não
-- manda a chave cria pedido normal (grava NULL; índice parcial ignora).
-- SEM BEGIN/COMMIT (regra da casa).
--
-- FICHA DE VERIFICAÇÃO pos-aplicação (produto real, sem variação, com
-- estoque; CEP local da loja; loja com frete local configurado):
--   BEGIN;
--     -- 1. MESMA chave 2x => MESMO pedido (a cura da 38, agora executável):
--     SELECT public.create_marketplace_order_v23(
--       '[{"product_id":"<uuid>","variant_id":null,"quantity":1}]'::jsonb,
--       <preco>, <frete-local>, 'na_entrega', NULL, NULL, 'Prova', '5531999999999',
--       NULL, '{"cep":"<cep-local>"}'::jsonb, '<cep-local>', 'local-delivery',
--       '<uuid-fixo>'::uuid);
--     -- repetir a MESMA chamada -> espera o MESMO uuid (antes: exception de INSERT)
--     -- 2. SEM chave (12 args; caminho do front que ainda não manda a chave)
--     --    cria pedido normal:
--     SELECT public.create_marketplace_order_v23(..., 'local-delivery');
--     -> espera nascer uuid (a função nova aceita a chamada antiga via DEFAULT)
--   ROLLBACK;
--
-- ROLLBACK MANUAL: versionado em
-- rollback-manual-20261040000000_a_idempotencia_insere_a_chave.sql

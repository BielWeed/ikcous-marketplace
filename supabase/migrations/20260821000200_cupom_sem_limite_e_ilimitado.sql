-- O cupom com "Limite de Uso" em branco volta a funcionar no fechamento do
-- pedido (auditoria de 20/08/2026, achado 1).
--
-- O DEFEITO, EM UMA FRASE
--   O painel promete "0 = Ilimitado" no formulario de cupom, a validacao do
--   checkout concorda, e a criacao do pedido discordava: com usage_limit = 0
--   o predicado `usage_count < usage_limit` e' `0 < 0`, falso para sempre.
--   Resultado medido no banco de desenvolvimento: quem digitava CUPOM10 via o
--   desconto de R$ 10,00 aplicado no total e levava "Cupom CUPOM10 invalido ou
--   expirado" ao tocar em finalizar. Venda perdida na ultima tela, com o app
--   acusando o cupom que ele mesmo tinha acabado de aceitar.
--
--   E o formulario empurra a loja para esse valor: o campo "Limite de Uso"
--   nasce em 0 e o texto de apoio diz "0 = Ilimitado"
--   (src/views/admin/AdminCouponFormView.tsx:35 e :430). Todo cupom criado sem
--   preencher aquele campo nascia quebrado.
--
-- POR QUE ALINHAR A CRIACAO DE PEDIDO, E NAO A VALIDACAO
--   Tres lugares dizem o que 0 significa: a tela, a `validate_coupon_secure_v2`
--   e estas duas funcoes. Dois deles ja diziam "ilimitado". Mudar o terceiro e'
--   a correcao que nao pede mudanca de dado, nao invalida cupom nenhum ja
--   gravado e nao muda o que a loja ja aprendeu na tela. O caminho oposto
--   — gravar NULL em vez de 0 — deixaria de pe todo cupom com 0 que ja existe,
--   aqui e em qualquer loja que nasca deste molde.
--
--   O predicado usa `<= 0` e nao `= 0` de proposito: e' a copia exata da regra
--   da `validate_coupon_secure_v2` (`usage_limit IS NOT NULL AND usage_limit > 0`),
--   que ja tratava limite negativo como ilimitado. Duas regras que decidem a
--   mesma coisa tem de ser identicas, ou o defeito volta pela outra ponta.
--
-- O QUE NAO MUDA
--   Limite de verdade continua valendo: com usage_limit = 1 e usage_count = 1
--   o cupom continua sendo recusado. O `FOR UPDATE` da linha, que e' a trava
--   contra dois pedidos gastarem a ultima unidade ao mesmo tempo, continua no
--   lugar. Nenhuma outra linha das duas funcoes foi tocada: o corpo abaixo e'
--   o `pg_get_functiondef` do que roda hoje, com uma unica substituicao por
--   funcao, conferida por contagem.
--
--   As legadas `create_marketplace_order` (sem sufixo) e
--   `validate_coupon_secure` (v1) tem o mesmo desencontro e NAO foram tocadas:
--   nenhuma delas e' chamada pelo app (o front chama v23, v24 e v2). Correcao
--   no caminho do dinheiro tem escopo minimo.
--
-- Sem BEGIN/COMMIT, de proposito: com eles o ROLLBACK do script de prova vira
-- no-op e a mudanca fica gravada mesmo assim.
-- Prova: node scripts/db-prove-cupom-sem-limite.cjs
...
COMMENT ON FUNCTION public.create_marketplace_order_v23(jsonb, numeric, numeric, text, uuid, text, text, text, text, jsonb, text, text) IS
'Cria pedido validando preco, estoque, frete e cupom no servidor. Limite de uso de cupom: NULL ou <= 0 significa ilimitado, a mesma regra da validate_coupon_secure_v2 e do que o painel promete com "0 = Ilimitado".';

COMMENT ON FUNCTION public.create_marketplace_order_v24(jsonb, numeric, numeric, text, uuid, text, text, text, text, jsonb, text, text) IS
'Igual a v23 no caminho do dinheiro; carimba payment_status=aguardando e expires_at para o pagamento online. Limite de uso de cupom: NULL ou <= 0 significa ilimitado, a mesma regra da validate_coupon_secure_v2 e do que o painel promete com "0 = Ilimitado".';

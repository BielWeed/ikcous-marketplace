-- A RETIRADA NA LOJA NASCE NO SERVIDOR (release 1.5.3, 22/09/2026).
--
-- O QUE A CLIENTE GANHA: quem é da área local da loja pode escolher
-- "Retirar na loja — grátis" (id "store-pickup") no carrinho. O pedido
-- nasce com frete ZERO e com o RETRATO do endereço físico da loja
-- (customer_data.pickup_address), que o painel, o detalhe do pedido e o
-- comprovante mostram. A escolha é SEMPRE da cliente — nada aqui a
-- seleciona sozinho.
--
-- CONTRATO (o mesmo da edge calculate-shipping e do front
-- src/lib/guarda-de-frete.ts — o id é o contrato, nunca o preço nem o
-- texto):
--   - id da opção: 'store-pickup'.
--   - liga/desliga: a MESMA chave 'store-pickup' dentro de
--     store_config.enabled_shipping_methods (text[], default '{sedex,pac}'
--     — a chave NÃO vem ligada em loja nenhuma). NULL, '{}' ou array sem a
--     chave = retirada DESLIGADA. Nas chaves de transportadora (sedex/pac/
--     jadlog…) nada muda: a edge ignora 'store-pickup' ao filtrar serviço.
--   - a opção só VALE com os TRÊS: chave habilitada + store_address (coluna
--     da 20261167000000) com btrim não vazio + CEP de entrega LOCAL
--     (public.is_local_cep, a mesma prova do 'local-delivery').
--   - pagamento: as MESMAS regras da entrega local (modalidade da loja):
--     v23 aceita os meios "na entrega", v24 aceita qualquer meio — nenhuma
--     restrição nova. Transportadora continua exigindo antecipado.
--
-- O QUE MUDA (NAS DUAS funções, create_marketplace_order_v23 e _v24; todo o
-- resto do corpo é BYTE A BYTE o que a 20261168000000 deixou — o preflight
-- abaixo prova o ponto de partida e o teste
-- tests/migration_retirada_na_loja_test.ts prova que só estas três trocas
-- existem):
--   (a) bloco 2-ter: ELSIF v_opcao = 'store-pickup' ANTES do ELSE
--       (transportadora) — revalida, fail-closed e ANTES do ramo do frete
--       grátis: id canônico (sem espaço de sobra), chave habilitada,
--       endereço físico preenchido, CEP de origem configurado e CEP de
--       entrega local. Frases de recusa JÁ classificadas pelo front
--       (src/lib/recusaDoPedido.ts): 'Opção de entrega inválida…',
--       'A loja ainda está configurando a entrega…' e 'Entrega local não
--       disponível para o CEP informado.' — cada uma com DETAIL próprio.
--   (b) bloco 4: ELSIF p_shipping_option_id = 'store-pickup' → frete 0,
--       ANTES do ramo do cache (retirada nunca é cotação gravada).
--   (c) INSERT: customer_data || {pickup_address: btrim(store_address)}
--       SÓ quando o id é 'store-pickup'; os outros pedidos nascem com o
--       MESMO customer_data de hoje (|| '{}' não muda nada).
--
-- DADOS EXISTENTES: nenhuma linha é lida para ser reescrita, nenhuma linha
-- é gravada. Pedido antigo fica como está; store_config fica como está
-- (retirada DESLIGADA em toda loja até o lojista ligar em Admin → Frete →
-- "Permitir retirada na loja", que exige o endereço de Admin → Sobre a
-- Loja). Sem coluna nova, sem GRANT, sem DROP: CREATE OR REPLACE preserva
-- dono e ACL (EXECUTE só para anon/authenticated/service_role).
--
-- PRÉ-REQUISITOS POR BANCO (o preflight ABORTA antes de qualquer mudança se
-- faltar um — nada fica pela metade):
--   - 20261167000000 aplicada (coluna store_config.store_address). Banco
--     sem ela: "aplique antes a 20261167000000_sobre_a_loja_ganha_endereco_
--     e_descricao.sql". Esta migration NÃO cria a coluna.
--   - 20261168000000 aplicada (os corpos que esta migration substitui). O
--     preflight aceita o corpo da 20261168 com fim de linha LF (aplicação
--     pelo CI Linux, banco efêmero) OU CRLF (como está hoje no banco vivo das
--     lojas — medido 22/09/2026: v23 2d99adfd…, v24 b728cea3…, iguais nos
--     dois bancos). Aceita também o PRÓPRIO corpo desta migration (LF ou
--     CRLF): reaplicar é no-op, não quebra.
--   - exatamente UM overload de cada função (13 argumentos).
--
-- ORDEM DE IMPLANTAÇÃO: banco → functions (calculate-shipping,
-- melhor-envio-etiqueta, send-order-confirmation/webhook que usam o
-- comprovante) → front. Com o banco novo e a edge velha, ninguém recebe a
-- opção (a edge velha não a produz) — janela inofensiva. Ligar a retirada
-- de uma loja SÓ depois da edge nova no ar: a edge velha trataria a chave
-- 'store-pickup' como transportadora e, num array só com ela, desligaria
-- as transportadoras.
--
-- ATOMICIDADE: SEM BEGIN/COMMIT (regra da casa). scripts/db-apply.cjs
-- aplica o arquivo inteiro numa transação — preflight e os dois CREATE OR
-- REPLACE entram juntos ou nenhum entra.
--
-- PROVA DE COMPORTAMENTO: tests/banco/invariantes-dinheiro.cjs (CI
-- rpc-ci.yml, Postgres efêmero) — retirada habilitada nasce com frete 0 e
-- pickup_address; desligada, sem endereço, fora da área, id com espaço e
-- array com NULL são recusados; a entrega local continua cobrando a taxa.
--
-- ROLLBACK MANUAL: rollback-manual-20261169000000_a_retirada_na_loja_nasce_no_servidor.sql
-- (reaplica os corpos da 20261168000000). Aplicar PELO PSQL, transação
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'PRE_REQUISITO_AUSENTE: a coluna public.store_config.store_address não existe neste banco — aplique antes a 20261167000000_sobre_a_loja_ganha_endereco_e_descricao.sql (esta migration não cria a coluna).';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
     WHERE table_schema = 'public' AND table_name = 'store_config'
       AND column_name = 'enabled_shipping_methods' AND udt_name = '_text'
  ) THEN
    RAISE EXCEPTION 'PRE_REQUISITO_AUSENTE: a coluna public.store_config.enabled_shipping_methods (text[]) não existe neste banco — a retirada na loja liga e desliga por ela.';
  END IF;
  ...
  -- Corpo da 20261168000000 (LF | CRLF) ou o desta migration (LF | CRLF).
  IF v_hash_v23 IS NULL OR v_hash_v23 NOT IN (
    ...
    RAISE EXCEPTION 'B1_BASELINE_DIVERGENT: corpo vivo da create_marketplace_order_v23 difere do que a 20261168000000 deixou (hash %) — aplique antes a 20261168000000 ou capture e revise', COALESCE(v_hash_v23, 'ausente');
  END IF;
  IF v_hash_v24 IS NULL OR v_hash_v24 NOT IN (
    ...
    RAISE EXCEPTION 'B1_BASELINE_DIVERGENT: corpo vivo da create_marketplace_order_v24 difere do que a 20261168000000 deixou (hash %) — aplique antes a 20261168000000 ou capture e revise', COALESCE(v_hash_v24, 'ausente');
  END IF;
END $preflight$;

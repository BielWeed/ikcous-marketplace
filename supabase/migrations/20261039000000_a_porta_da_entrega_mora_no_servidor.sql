-- A PORTA DA ENTREGA MORA NO SERVIDOR (laudo caça-bugs do molde, 31/08/2026,
-- achado A2 + item E).
--
-- O DEFEITO PROVADO: a regra do convidado (só entrega local — decisão do
-- Gabriel de 30/08) e a cobertura configurada da loja
-- (store_config.shipping_coverage) existiam SÓ na tela
-- (CheckoutView.tsx:884-889 era o portão inteiro). Chamando
-- create_marketplace_order_v23/v24 direto — e a grant EXECUTE a anon é
-- pública de propósito — QUALQUER CEP passava: o servidor nunca leu
-- shipping_coverage, e o caminho flat-fee-* nem olhava CEP. O comentário do
-- cep-local.ts ("a decisão final é do servidor") descrevia um servidor que
-- não existia. Junto, o item E: frete cotado no CEP A (a calculadora fica
-- no carrinho, com campo próprio) era cobrado para entregar no CEP B.
--
-- A CURA, em cada v23/v24 (enxertos sobre o corpo da 20261038000000 — esta
-- migration DEPENDE dela aplicar antes; ledger na ordem do nome):
--   2-bis. O portão, ANTES de qualquer cálculo de preço:
--     1. CEP DE ENTREGA verdadeiro: address_data.cep do convidado → cep do
--        endereço do logado (dono provado no passo 1) → cotação, por último.
--     2. RECONCILIAÇÃO: cotação e entrega em CEPs diferentes RECUSA —
--        "o frete foi cotado para outro CEP". Cotação ausente não decide.
--     3. CONVIDADO: só entrega local (is_local_cep); sem origem
--        configurada, o aviso é da loja (ela ainda nem vende — B1).
--     4. COBERTURA 'local': vale para TODOS — logado incluso, frete grátis
--        incluso; entrega desconhecida falha fechada.
--   FRONT do mesmo PR: o efeito de invalidação no CheckoutView derruba a
--   opção escolhida quando o destino muda (src/lib/reconciliacao-de-cep.ts)
--   — o cliente re-cota sem nunca ver a recusa; a recusa do servidor fica
--   para quem chama a RPC direto.
--
-- Corpos: VERBATIM da 20261038000000 com os 2 enxertos por função (prova
-- interna do gerador: reverter os enxertos devolve a 38 byte a byte).
-- CREATE OR REPLACE preserva GRANT/dono (lição 448 do _REGRAS); assinatura
-- repetida idêntica. SEM BEGIN/COMMIT (regra da casa).
--
-- FICHA DE VERIFICAÇÃO pos-aplicação (banco do molde; loja com origem
-- 38500000 configurada e produto real com estoque):
--   -- 1. CONVIDADO fora da área é recusado (controle do A2):
--   BEGIN;
--     SELECT public.create_marketplace_order_v23(
--       '[{"product_id":"<uuid>","variant_id":null,"quantity":1}]'::jsonb,
--       <preco>, 0, 'na_entrega', NULL, NULL, 'Prova A2', '5531999999999',
--       NULL, '{"cep":"01310100"}'::jsonb, '01310100', NULL, NULL);
--     -> espera ERRO 'Compra sem conta é só com entrega na cidade da loja.'
--   ROLLBACK;
--   -- 2. CONVIDADO na área passa (sem regressão do caminho legítimo):
--   BEGIN;
--     SELECT public.create_marketplace_order_v23(..., '{"cep":"38500000"}'::jsonb,
--       '38500000', NULL, NULL);
--     -> espera nascer pedido (uuid)
--   ROLLBACK;
--   -- 3. RECONCILIAÇÃO: cotação 01310100 × entrega 38500000 recusa:
--   BEGIN;
--     SELECT public.create_marketplace_order_v23(..., '{"cep":"38500000"}'::jsonb,
--       '01310100', NULL, NULL);

-- Top 5 "mais lucrativos" ordena por LUCRO, nao por faturamento.
--
-- Achado 2 da auditoria de banners/dashboard (degrau 1; veredicto ABERTO
-- no levantamento de 25/08, autorizado na divisao da mesa 1930): o bloco
-- top_products da get_admin_analytics_v2 ordenava por
-- SUM(oi.quantity * oi.price) - FATURAMENTO - enquanto o rótulo da tela
-- diz "mais lucrativos". Medido na auditoria: o 2o e o 3o do ranking
-- estavam trocados (maleta R$ 27,82 de lucro acima do Bobbie Goods
-- R$ 47,60); o lojista decide reposicao e anuncio em cima dessa ordem.
--
-- A correcao copia a convencao QUE A PROPRIA FUNCAO JA USA no bloco de
-- historico (linha ~223 da definicao vigente, e o padrao de tres
-- migrations): SUM(qty * (price - COALESCE(custo, 0))).
-- Produto sem custo cadastrado conta custo 0 (lucro = faturamento) -
-- cadastrar o custo corrige o ranking; declarado em vez de escondido.
--
-- Tecnica: definicao VERBATIM da vigente (20260902000000) com UMA linha
-- trocada - diff de 1 linha conferido por script antes de gravar. O
-- front nao muda: o campo `total` passa a carregar lucro e o rotulo
-- "mais lucrativos" passa a ser verdade.
--
-- Expand/contract ok: so o significado de um campo json melhora; nada
-- de schema. SEM BEGIN/COMMIT. Faixa 20261000* (cacador-b-dorso,
-- reservada no _REGRAS.md). NAO aplicar sem prova de ROLLBACK e sem o
...
--    LIKE '%COALESCE(p.custo, 0))) as total%';
--     -> espera true  (controle negativo: LIKE '%oi.price) as total%' -> false)
--   E no painel: a ordem do top-5 bate com ORDER BY lucro medido no banco.

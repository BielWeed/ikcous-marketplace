-- FORMAS DE PAGAMENTO POR LOJA (25/09/2026, com revisão de desenho de um
-- crítico externo no mesmo dia — itens B1/B2/B3/A1/A7 abaixo já incorporados
-- ao corpo desta migration, não a um rascunho anterior).
--
-- O QUE FALTAVA: o lojista queria vender SÓ com pagamento pelo app (evita
-- reserva e cancelamento na hora da entrega), mas pix/cartão/dinheiro NA
-- ENTREGA eram uma lista FIXA no front (`CheckoutView.tsx`, `opcoesNaEntrega`)
-- — nenhuma configuração por loja, e o SERVIDOR não validava a forma de
-- pagamento nenhuma (`create_marketplace_order_v23`/`_v24` aceitavam
-- qualquer `p_payment_method`). "online" (PIX pelo app) já ligava/desligava
-- por `store_config.pagamento_online` (edge `credenciais-mercado-pago`); as
-- outras três formas não tinham interruptor nenhum.
--
-- O QUE ESTA MIGRATION FAZ, NA ORDEM:
--   1. `store_config.formas_pagamento_entrega text[] NOT NULL DEFAULT
--      ARRAY['pix','card','cash']` — ADITIVA: toda loja existente nasce com
--      as três formas ligadas (comportamento IDÊNTICO ao de hoje; nenhuma
--      loja muda sozinha). Duas CHECK constraints: só elementos em
--      ('pix','card','cash') (`<@`, sem subquery — CHECK não aceita
--      subquery de tabela) e sem duplicata, via a função IMMUTABLE nova
--      `public.formas_pagamento_sem_duplicata` (subquery É permitida DENTRO
--      do corpo de uma função; só não dentro do texto do CHECK).
--   2. `public.forma_de_pagamento_aceita(p_metodo text) RETURNS boolean`
--      (SECURITY DEFINER, STABLE, search_path fixo) — fonte única: 'online'
--      olha `pagamento_online`; 'pix'/'card'/'cash' olham
--      `formas_pagamento_entrega`; qualquer outro valor devolve `false`.
--   3. `public.store_config_exige_forma_de_pagamento()` — trigger BEFORE
--      INSERT OR UPDATE em `store_config`: recusa (`RAISE EXCEPTION
--      'LOJA_SEM_FORMA_DE_PAGAMENTO'`) quando a linha final ficaria com
--      `cardinality(formas_pagamento_entrega) = 0 AND NOT pagamento_online`
--      — a loja sempre precisa de PELO MENOS uma forma (na entrega OU pelo
--      app). Mensagem SEM texto em português (o texto amigável mora em quem
--      consome — edge e front —, não na trigger; `error.message` do
--      Postgres carrega exatamente esse literal).
--      🔴 B1 (achado do crítico): `upsert_store_config` é
--      `INSERT ... ON CONFLICT DO UPDATE`, e o Postgres roda a trigger
--      BEFORE INSERT sobre o CANDIDATO do INSERT ANTES de decidir se colide
--      — mesmo quando o destino real é o UPDATE (id=1 quase sempre já
--      existe). O candidato do INSERT NÃO é a linha final:
--      `pagamento_online` nem entra na lista de colunas graváveis de
--      `upsert_store_config` (carrega o DEFAULT `false`, nunca o valor real
--      da loja). Validar esse candidato fantasma travaria QUALQUER
--      salvamento de uma loja que já vende só pelo app
--      (`formas_pagamento_entrega = '{}'` com `pagamento_online = true` é
--      estado VÁLIDO — é o objetivo desta funcionalidade). O conserto tem
--      DUAS pernas: (a) a trigger PULA a validação no disparo BEFORE INSERT
--      quando a linha `NEW.id` já existe (`EXISTS(SELECT 1 FROM
--      store_config WHERE id = NEW.id)`) — quem valida de verdade nesse
--      caso é o disparo BEFORE UPDATE que o `ON CONFLICT` aciona a seguir,
--      sobre a linha final de verdade; (b) o candidato do INSERT em
--      `upsert_store_config`, quando o payload NÃO manda
--      `formas_pagamento_entrega`, usa o valor ATUAL da linha (mesmo truque
--      do `logo_url`, `SELECT ... FROM store_config WHERE id=1`) em vez de
--      um array fixo — defesa em profundidade, para o dia em que o INSERT
--      genuíno (loja id=1 nascendo agora) coincidir com um payload parcial.
--      Loja GENUINAMENTE nova passa sempre: o DEFAULT da coluna já são as
--      3 formas.
--   4. `v_store_config`: a coluna nova no FIM (`DROP VIEW` + `CREATE VIEW`
--      — a lista de colunas é IDÊNTICA à que a 20261171000000 deixou, + 1).
--   5. `upsert_store_config`: aceita o campo novo seguindo o padrão parcial
--      já usado por toda coluna desta RPC (`CASE WHEN config_json ?
--      'coluna' THEN ... ELSE store_config.coluna END` no `ON CONFLICT DO
--      UPDATE`) — quem não manda o campo não o altera. Resto do corpo BYTE A
--      BYTE igual ao que a 20261171000000 deixou (preflight abaixo prova o
--      ponto de partida pelo hash do `prosrc`).
--   6. `create_marketplace_order_v23`/`_v24`: chamam
--      `public.forma_de_pagamento_aceita(p_payment_method)` logo DEPOIS do
--      passo 0 (idempotência) e ANTES do passo 1 (posse do endereço) — RAISE
--      'Esta forma de pagamento não está disponível nesta loja. Escolha
--      outra.' quando recusada.
--      🔴 B3 (achado do crítico): DEPOIS da idempotência, de propósito — um
--      retry com a MESMA chave (`p_idempotency_key`) tem de devolver o
--      pedido que JÁ NASCEU, mesmo que a loja tenha desligado esta forma de
--      pagamento DEPOIS da compra confirmada; só pedido NOVO passa pela
--      checagem. A mensagem é TEXTO PURO, sem prefixo de código — ao
--      contrário de `FRETE_COTACAO_DESATUALIZADA` (que tem tratamento
--      PRÓPRIO no front, fora de `recusaDoPedido.ts`), esta segue o padrão
--      predominante daquele arquivo: casada por regex sobre `message`
--      inteiro, nunca por ERRCODE nem prefixo (ver a regra nova em
--      `src/lib/recusaDoPedido.ts`, adicionada na mesma rodada desta
--      migration). Resto dos dois corpos é BYTE A BYTE o que a
--      20261172000000 (CPF do destinatário) deixou — preflight abaixo prova
--      o ponto de partida.
--
-- DADOS EXISTENTES: `ADD COLUMN ... NOT NULL DEFAULT ARRAY['pix','card',
-- 'cash']` preenche TODA linha existente (aqui, só a `id=1`) com as três
-- formas — o mesmo comportamento de hoje, sem UPDATE explícito nenhum (o
-- backfill é do próprio `ALTER TABLE`). Nenhuma outra tabela é lida nem
-- reescrita.
--
-- IDEMPOTÊNCIA: `ADD COLUMN IF NOT EXISTS`, `DO $$ IF NOT EXISTS...` para as
-- duas CHECK constraints, `CREATE OR REPLACE` para as três funções e a
-- trigger (via `DROP TRIGGER IF EXISTS` + `CREATE TRIGGER`), `DROP VIEW` +
-- `CREATE VIEW` para a view (mesmo padrão da 20261171000000, que não é
-- `CREATE OR REPLACE`-ável ao acrescentar coluna no meio de uma lista
-- fixa — aqui a coluna entra no FIM, mas o padrão da casa já usa DROP+CREATE
-- para view mesmo assim). Reaplicar este arquivo é seguro: o preflight abaixo
-- aceita tanto o corpo QUE ESTA MIGRATION SUBSTITUI quanto o corpo QUE ELA
-- PRÓPRIA deixa (para a segunda aplicação encontrar o próprio resultado).
--
-- FORA DO ESCOPO: aplicar de verdade em qualquer banco (o revisor Opus e o
-- Gabriel decidem isso depois, como toda migration desta casa); o PDV
-- (`registrar_venda_presencial`, fora do pedido) e cartão online (Fase 3.5,
-- código morto) continuam intocados; o aviso do painel administrativo sobre
-- "todas as formas na entrega desligadas" é código de FRONT, fora desta
-- migration.
--
-- COMO APLICAR: `node scripts/db-apply.cjs <arquivo.sql>` (sem BEGIN/COMMIT
...
--      -- esperado: ARRAY, NOT NULL, default '{pix,card,cash}'::text[]
--   2. SELECT formas_pagamento_entrega FROM public.v_store_config;
--      -- esperado: {pix,card,cash} (loja existente, sem migration de dado
--      -- nenhuma além do backfill do ALTER TABLE)
--   3. SELECT public.forma_de_pagamento_aceita('pix'),
--             public.forma_de_pagamento_aceita('online'),
--             public.forma_de_pagamento_aceita('whatsapp');
--      -- esperado: true, (pagamento_online da loja), false
--   4. UPDATE public.store_config SET formas_pagamento_entrega = '{}'
--        WHERE id=1 AND pagamento_online = true;  -- esperado: sucesso
--      UPDATE public.store_config SET formas_pagamento_entrega = '{}'
--        WHERE id=1 AND pagamento_online = false; -- esperado: RAISE
--        LOJA_SEM_FORMA_DE_PAGAMENTO
--   5. `SELECT public.upsert_store_config('{"free_shipping_min": 10}'::jsonb);`
--      numa loja com `formas_pagamento_entrega = '{}'` e
--      `pagamento_online = true` — esperado: sucesso, sem tocar
--      `formas_pagamento_entrega` (B1 — o candidato fantasma do INSERT não
--      trava um salvamento não relacionado).
--
-- ROLLBACK MANUAL:
-- rollback-manual-20261174000000_formas_de_pagamento_por_loja.sql (restaura
-- os corpos EXATOS que esta migration substituiu — v23, v24,
-- upsert_store_config e a view sem a coluna nova — dropa a trigger, a
-- função `forma_de_pagamento_aceita`, o helper de duplicata e as duas CHECK
-- constraints; NÃO dropa a COLUNA `formas_pagamento_entrega` — apagaria a
-- configuração que a lojista já salvou, mesma régua da rollback-manual-
-- 20261171000000). Aplicar PELO PSQL, transação única — nunca pelo
-- db-apply (registraria o rollback no ledger de migrations).
...
BEGIN
  ...
  -- Corpo que a 20261172000000 (CPF do destinatário) deixou (LF | CRLF) ou o
  -- corpo QUE ESTA MIGRATION deixa (LF | CRLF -- reaplicação idempotente).
  IF v_hash_v23 IS NULL OR v_hash_v23 NOT IN (
    ...
    RAISE EXCEPTION 'B1_BASELINE_DIVERGENT: corpo vivo da create_marketplace_order_v23 difere do que a 20261172000000 deixou (hash %) -- aplique antes a 20261172000000 ou capture e revise', COALESCE(v_hash_v23, 'ausente');
  END IF;
  IF v_hash_v24 IS NULL OR v_hash_v24 NOT IN (
    ...
    RAISE EXCEPTION 'B1_BASELINE_DIVERGENT: corpo vivo da create_marketplace_order_v24 difere do que a 20261172000000 deixou (hash %) -- aplique antes a 20261172000000 ou capture e revise', COALESCE(v_hash_v24, 'ausente');
  END IF;

  -- Corpo que a 20261171000000 (frete nacional) deixou (LF | CRLF) ou o
  -- corpo QUE ESTA MIGRATION deixa (LF | CRLF).
  IF v_hash_upsert IS NULL OR v_hash_upsert NOT IN (
    ...
    RAISE EXCEPTION 'B1_BASELINE_DIVERGENT: corpo vivo da upsert_store_config difere do que a 20261171000000 deixou (hash %) -- aplique antes a 20261171000000 ou capture e revise', COALESCE(v_hash_upsert, 'ausente');
  END IF;
END $preflight$;

-- (1) A coluna nova -- ADITIVA, backfill automático do ALTER TABLE (não é
-- UPDATE explícito): toda loja existente nasce com as três formas ligadas.
ALTER TABLE public.store_config
  ADD COLUMN IF NOT EXISTS formas_pagamento_entrega text[] NOT NULL DEFAULT ARRAY['pix','card','cash']::text[];

-- Helper IMMUTABLE para a checagem de duplicata (A1: CHECK não aceita
-- subquery de TABELA no seu próprio texto; uma função pode ter subquery no
-- CORPO dela, e o CHECK só enxerga a chamada da função).
...
PARALLEL SAFE
AS $$
  SELECT COALESCE(array_length(arr, 1), 0) = (SELECT count(DISTINCT x) FROM unnest(arr) AS x);
$$;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
     WHERE conrelid = 'public.store_config'::regclass
       AND conname = 'store_config_formas_pagamento_entrega_check'
  ) THEN
    ALTER TABLE public.store_config
      ADD CONSTRAINT store_config_formas_pagamento_entrega_check
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
     WHERE conrelid = 'public.store_config'::regclass
       AND conname = 'store_config_formas_pagamento_sem_duplicata_check'
  ) THEN
    ALTER TABLE public.store_config
      ADD CONSTRAINT store_config_formas_pagamento_sem_duplicata_check
      CHECK (public.formas_pagamento_sem_duplicata(formas_pagamento_entrega));
  END IF;
END $$;

-- (2) Fonte única: quem decide se um método de pagamento está ligado nesta
-- loja. Chamada por create_marketplace_order_v23/_v24 (abaixo).
...
DECLARE
  v_config RECORD;
BEGIN
  SELECT pagamento_online, formas_pagamento_entrega INTO v_config
    FROM public.store_config WHERE id = 1;
  IF NOT FOUND THEN
    RETURN false;
  END IF;
  RETURN CASE p_metodo
    WHEN 'online' THEN v_config.pagamento_online
    WHEN 'pix' THEN 'pix' = ANY(v_config.formas_pagamento_entrega)
    WHEN 'card' THEN 'card' = ANY(v_config.formas_pagamento_entrega)
    WHEN 'cash' THEN 'cash' = ANY(v_config.formas_pagamento_entrega)
    ELSE false
  END;
END;
$function$;

-- (3) O invariante: a loja sempre tem AO MENOS uma forma de pagamento --
-- na entrega, ou pelo app. B1 (ver cabeçalho): o disparo BEFORE INSERT PULA
-- a validação quando a linha id=NEW.id já existe -- o candidato de um
-- INSERT que vai desviar para ON CONFLICT DO UPDATE não é a linha final, e
-- validar esse fantasma travaria salvamentos legítimos de uma loja que já
-- vende só pelo app. O disparo BEFORE UPDATE (o que o ON CONFLICT aciona de
-- fato) valida a linha final de verdade, sempre.
...
BEGIN
  IF TG_OP = 'INSERT' AND EXISTS (
    SELECT 1 FROM public.store_config WHERE id = NEW.id
  ) THEN
    RETURN NEW;
  END IF;

  IF cardinality(NEW.formas_pagamento_entrega) = 0 AND NOT NEW.pagamento_online THEN
    RAISE EXCEPTION 'LOJA_SEM_FORMA_DE_PAGAMENTO';
  END IF;
...
REVOKE ALL ON FUNCTION public.store_config_exige_forma_de_pagamento() FROM PUBLIC, anon, authenticated, service_role;

DROP TRIGGER IF EXISTS store_config_exige_forma_de_pagamento ON public.store_config;
CREATE TRIGGER store_config_exige_forma_de_pagamento
  BEFORE INSERT OR UPDATE ON public.store_config
  FOR EACH ROW
  EXECUTE FUNCTION public.store_config_exige_forma_de_pagamento();

-- (4) v_store_config: a lista da 20261171000000, na MESMA ordem, + a coluna
-- nova no FIM. DROP + CREATE (não OR REPLACE) -- mesmo padrão da
-- 20261171000000 para troca de lista de colunas.
...
-- (5) upsert_store_config: BYTE A BYTE o corpo que a 20261171000000 deixou
-- + o splice do B1 (ver cabeçalho) -- preflight acima provou o ponto de
-- partida.
...
-- (6) create_marketplace_order_v23: BYTE A BYTE o corpo que a 20261172000000
-- (CPF do destinatário) deixou + o splice do B3 (ver cabeçalho) -- preflight
-- acima provou o ponto de partida.
...
BEGIN
    ...
    -- 0-bis. FORMA DE PAGAMENTO DESLIGADA NA LOJA (formas de pagamento por
    -- loja, 25/09/2026 — revisão do desenho por um crítico externo, item B3):
    -- DEPOIS do passo 0 (idempotência), de propósito — o retry de um pedido
    -- que JÁ EXISTE (mesma chave) tem de voltar o pedido que já nasceu, mesmo
    -- que a loja tenha desligado esta forma de pagamento DEPOIS da compra
    -- confirmada; só pedido NOVO passa por aqui.
    -- public.forma_de_pagamento_aceita(p_metodo) é a fonte única: 'online'
    -- olha store_config.pagamento_online; 'pix'/'card'/'cash' olham
    -- store_config.formas_pagamento_entrega; qualquer outro valor de
    -- p_payment_method devolve false. A mensagem é TEXTO puro, sem prefixo
    -- de código — mesmo padrão de recusaDoPedido.ts (ver o cabeçalho daquele
    -- arquivo): casada por regex sobre `message`, nunca por ERRCODE.
...
-- (7) create_marketplace_order_v24: BYTE A BYTE o corpo que a 20261172000000
-- (CPF do destinatário) deixou + o splice do B3 (ver cabeçalho) -- preflight
-- acima provou o ponto de partida.

-- ROLLBACK MANUAL de 20261171000000_o_frete_nacional_ganha_estrategia_propria.sql
-- (o frete nacional ganha estratégia própria -- T1/banco, estratégias de
-- frete local e nacional, 23/09/2026)
--
-- Devolve as TRÊS funções ao corpo EXATO de antes desta migration:
--   - create_marketplace_order_v23/_v24: o corpo que a 20261170000000
--     deixou (a checagem '_revisao', sem a regra nacional desta migration).
--   - upsert_store_config: o corpo que a 20261167000000 deixou (sem as 5
--     colunas nacionais no INSERT/ON CONFLICT).
--   - v_store_config: as 34 colunas da 20261167000000, SEM as 5 nacionais
--     no fim.
-- ORDEM: as três funções primeiro (voltam à regra de ontem), a view por
-- último (para de expor as colunas nacionais na leitura pública).
--
-- 🔴 NÃO DROPA AS 5 COLUNAS NACIONAIS. Diferente do rollback da
-- 20261167000000 (que apaga store_address/store_description — molde
-- desta), este arquivo PRESERVA as colunas: DROP COLUMN apagaria a
-- estratégia de frete nacional que a lojista já configurou na tela nova
-- (national_shipping_strategy, min, discount_type/value, benefit_scope) —
-- exatamente o dado que a checagem do "espelho legado" (bloco 4 das duas
-- RPCs, nesta migration) existe para preservar contra reescrita. Reverter
-- só a REGRA (as três funções + a view) e manter a COLUNA é reversível dos
-- dois lados: reaplicar esta migration depois de um rollback encontra a
-- coluna já lá (a guarda por information_schema.columns não copia de novo
-- — a config da lojista continua intacta) e volta a validar o carimbo
-- normalmente.
--
-- POR QUE `DROP VIEW` + `CREATE VIEW` (nunca `CREATE OR REPLACE VIEW`), no
-- molde do rollback da própria 20261167000000: a view viva no momento
-- deste rollback tem 39 colunas (as 34 da 20261167000000 + as 5 desta
-- migration) e o Postgres RECUSA `OR REPLACE` que encolha a lista ("cannot
-- drop columns from view"). `DROP VIEW` apaga o ACL junto, por isso o
-- GRANT explícito logo depois — o MESMO `GRANT ALL ON TABLE ... TO anon,
-- authenticated, service_role` que os rollbacks de 20261121000000,
-- 20261140000000, 20261150000000 e 20261167000000 já usam para esta mesma
-- view (grant vigente, medido nesses quatro arquivos) — sem ele a view
-- voltaria a existir sem os grants que anon/authenticated/service_role
-- tinham, e o rollback deixaria de ser fiel.
--
-- MODO DE APLICAÇÃO -- psql, UMA transação externa ÚNICA:
--   psql "$DATABASE_URL" -1 -f rollback-manual-20261171000000_o_frete_nacional_ganha_estrategia_propria.sql
-- As três funções e a view voltam JUNTAS ou nada volta. SEM BEGIN/COMMIT
-- dentro do arquivo (regra da casa). NUNCA pelo scripts/db-apply.cjs (ele
-- registraria o rollback no ledger de migrations).
--
-- JANELA DE PUBLICAÇÃO (ordem inversa da aplicação): reverta a EDGE
-- (calculate-shipping) para a versão anterior a este pacote ANTES ou
-- DEPOIS deste rollback não importa aqui -- diferente da 20261170000000,
-- esta checagem não vira mais restritiva sem carimbo: cotação sem o campo
-- `estrategiaNacional` (edge velha OU nova, tanto faz depois deste
-- rollback) deixa de ser cobrada por esta migration -- o comportamento
-- volta a ser o de antes (free_shipping_min decidindo por QUALQUER id, via
-- create_marketplace_order_v23/_v24 já revertidas acima).
--
-- Preflight: o corpo vivo das três funções tem de ser o desta migration
-- (LF ou CRLF) -- ou já o corpo alvo do rollback (LF ou CRLF: rollback
-- repetido é no-op). Qualquer outro corpo para tudo antes de mexer em
-- qualquer coisa.
DO $preflight$
DECLARE
  v_hash_v23 text;
...
BEGIN
  ...
  IF v_hash_v23 IS NULL OR v_hash_v23 NOT IN (
    ...
    RAISE EXCEPTION 'B1_BASELINE_DIVERGENT: corpo vivo da create_marketplace_order_v23 não é o desta migration nem o da 20261170000000 (hash %) -- capture e revise antes de reverter', COALESCE(v_hash_v23, 'ausente');
  END IF;
  IF v_hash_v24 IS NULL OR v_hash_v24 NOT IN (
    ...
    RAISE EXCEPTION 'B1_BASELINE_DIVERGENT: corpo vivo da create_marketplace_order_v24 não é o desta migration nem o da 20261170000000 (hash %) -- capture e revise antes de reverter', COALESCE(v_hash_v24, 'ausente');
  END IF;
  IF v_hash_upsert IS NULL OR v_hash_upsert NOT IN (
    ...
    RAISE EXCEPTION 'B1_BASELINE_DIVERGENT: corpo vivo da upsert_store_config não é o desta migration nem o da 20261167000000 (hash %) -- capture e revise antes de reverter', COALESCE(v_hash_upsert, 'ausente');
  END IF;
END $preflight$;

-- (1) create_marketplace_order_v23/_v24: corpo EXATO que a 20261170000000
-- deixou (cópia literal do CREATE OR REPLACE dela — a mesma prova por hash
-- do preflight acima garante que este texto bate com o que fica no banco).
...
-- (2) upsert_store_config: corpo EXATO que a 20261167000000 deixou (sem as
-- 5 colunas nacionais).
...
-- (3) View sem as 5 colunas nacionais (as 34 da 20261167000000, mesma
-- ordem).
